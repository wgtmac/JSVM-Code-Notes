...
ErrVal ParameterSetMng::store( SequenceParameterSet* pcSPS )
{
  ...
  {
    uiSPSId += NUM_SPS_IDS;              //SPS的的id为0-31  这是用来区别subset的   它存在index=32起的缓存中
  }
...
ErrVal ParameterSetMng::store( PictureParameterSet* pcPPS )
{
  ...
  ROF( m_cPPSBuf.isValidOffset( uiPPSId ) )          //这个参数集管理类有 StatBuf<PictureParameterSet*,256> m_cPPSBuf;  存储PPS

  RNOK( xDestroyPPS( uiPPSId ) );     //将缓存处的PPS存入list中
...
ErrVal ParameterSetMng::xDestroyPPS(UInt uiPPSId)
{
  ...
  RNOK( m_cPPSBuf.get( pcPPS, uiPPSId ) );    //从参数集管理类中的Buf中读取出对应PPS

  ROTRS( NULL == pcPPS, Err::m_nOK );

  if( pcPPS )
  {
    m_cPPSList.push_back( pcPPS );        //将对应的PPS存入参数集管理类中的list中
  }

  m_cPPSBuf.set( uiPPSId, NULL );       //原缓存处置为空
  return Err::m_nOK;
}
