...
UInt MbData::calcMbCbp( UInt uiStart, UInt uiStop ) const
{
  ...
  if( isIntra16x16() )                  // INTRA_16x16没有CBP
  {
  ...
  else
  {
    uiCbp = ( ( uiCbp & 0x0033 ) ?  1 : 0 ) |
            ...
            ( ( uiCbp & 0x3300 ) ?  4 : 0 ) |
            ( ( uiCbp & 0xcc00 ) ?  8 : 0 ) |
            ( bChromaAC          ? 32 : ( bChroma ? 16 : 0 ) );                //其他模式CBP 每4个4x4块才决定一个8x8块
  }
  return uiCbp;
}
                    ...
                    UInt    uiSliceId )
{
  ...
  m_aBlkMode[0] = rcMbData.m_aBlkMode[0];               // 每个8x8块的划分
  ...
  m_apcMbMotionData[0]->copyFrom( *rcMbData.m_apcMbMotionData[0] );    // 拷贝 ref_idx
  m_apcMbMotionData[1]->copyFrom( *rcMbData.m_apcMbMotionData[1] );

  m_apcMbMvdData[0]->copyFrom( *rcMbData.m_apcMbMvdData[0] );              // 拷贝 mvd
  m_apcMbMvdData[1]->copyFrom( *rcMbData.m_apcMbMvdData[1] );
