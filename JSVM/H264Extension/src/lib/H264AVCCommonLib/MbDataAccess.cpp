...
//按 ePredType 方式获得list(eListIdx) 的MVP ===> rcMvPred
Void MbDataAccess::xGetMvPredictor( Mv& rcMvPred, SChar scRef, ListIdx eListIdx, PredictionType ePredType, LumaIdx cIdx, LumaIdx cIdxEnd )
{
  ...
  if( m_cMv3D_C == BLOCK_NOT_AVAILABLE )      // 右上角不可用 则用左上角代替
  {
  ...
  //===== check directional prediction types =====       A B C的ref_idx要和现在传入的scRef一样才能使用该MV
  ...
  {
    rcMvPred = m_cMv3D_A;                    // 只采用 A 块预测
    return;
  }
  if( ( ePredType == PRED_B && m_cMv3D_B == scRef ) ||
      ( m_cMv3D_A != scRef  && m_cMv3D_B == scRef && m_cMv3D_C != scRef ) )
  {
    rcMvPred = m_cMv3D_B;                    // 只采用 B 块预测
    return;
  }
  if( ( ePredType == PRED_C && m_cMv3D_C == scRef ) ||
      ( m_cMv3D_A != scRef  && m_cMv3D_B != scRef && m_cMv3D_C == scRef ) )
  {
    rcMvPred = m_cMv3D_C;                    // 只采用 C 块预测
    return;
  }
    // 取 A B C三个块Mv的中间值
...
{
  xGetMvPredictor( rcMvPred, 1, LIST_0, MEDIAN, B4x4Idx(0), B4x4Idx(3) );          //按 MEDIAN 方式获得LIST_0 中ref_idx=1的MVP ===>rcMvPred
