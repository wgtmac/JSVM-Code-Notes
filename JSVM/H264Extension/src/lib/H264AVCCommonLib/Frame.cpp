...
{
  ASSERT( m_ePicType==FRAME );
  RNOK( getFullPelYuvBuffer()->loadFromPicBuffer( pcPicBuffer ) );         //将原始图像从PicBuffer读入YuvPicBuffer结构
  return Err::m_nOK;
}
...
ErrVal Frame::extendFrame( QuarterPelFilter* pcQuarterPelFilter, PicType ePicType, Bool bFrameMbsOnlyFlag )
{
	...
	if( NULL != m_pcFrameTopField || NULL != m_pcFrameBotField )          // FRAME 情况下都是 NULL
  {
  ...
  m_bExtended     = true;
	...
	if( ! bFrameMbsOnlyFlag )              //下面是场内容 不需要看了 ******************************************************
	{
		if( ePicType==FRAME )
		{
...
//TMM }
  ...
  // create half pel samples               进行int=>half的插值
...
Frame::setPicParameters( const PictureParameters& rcPP, PicType ePicType )
{
  ...
  {
    m_cPicParameters    = rcPP;       //顶场的参数
  }
  if( ePicType == FRAME || ePicType == BOT_FIELD )
  {
    m_cPicParametersBot = rcPP;     //底场的参数
  }
