...
Void YuvMbBuffer::loadIntraPredictors( const YuvPicBuffer* pcSrcBuffer )
{
  ...
  pSrc -= iSrcStride+1;                //  上面一行最左边 再往左边一个 的点
  pDes -= iDesStride+1;

  memcpy( pDes, pSrc, sizeof(XPel)*21 );               //   1 + 16 + 4     当前宏块上面一行
  memcpy( pDes+iDesStride+17, pSrc+21, sizeof(XPel)*4 );     

  for( y = 0; y < 16; y++)              //当前宏块左边一列
  {
    pSrc += iSrcStride;
    pDes += iDesStride;
    *pDes = *pSrc;
...
Void YuvMbBuffer::loadBuffer( const YuvPicBuffer* pcSrcBuffer )
{
  ...
  pSrc = pcSrcBuffer->getMbLumAddr();           //获得 YuvPicBuffer 的 luma 地址
  pDes = getMbLumAddr();                                   //获得 YuvMbBuffer 的 luma 地址
  ...
  pSrc = pcSrcBuffer->getMbCbAddr();
  pDes = getMbCbAddr();
  iDesStride = getCStride();                //luma 和 chroma 一样  都是24
  iSrcStride = pcSrcBuffer->getCStride();
