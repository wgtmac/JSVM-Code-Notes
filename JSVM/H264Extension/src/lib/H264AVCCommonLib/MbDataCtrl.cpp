...
ErrVal
MotionUpsampling::resample( Int iMbXCurr, Int iMbYCurr )     //重采样，其实就是得到BL的信息
{
  ...
  if( m_bInCropWindow && ! m_bIntraBL )
  {
    ...
    {
      RNOK( xGetRefIdxAndInitialMvPred( ListIdx( iListIdx ) ) );    //获得BaseLayer的ref_idx和Mv
    }
    for( Int iB8x8Idx = 0; iB8x8Idx < 4; iB8x8Idx++ )
    {
      RNOK( xDeriveBlockModeAndUpdateMv( iB8x8Idx ) );      //根据宏块分区更新Mv
    }
    RNOK( xDeriveMbMode       () );    //由BaseMbMode检查ref_idx和Mv，并得到MbMode
    RNOK( xDeriveFwdBwd       () );
                                  ...
                                  Int&  riYInsideBaseMb )
{
  ...
  //===== get virtual MbData =====   得到当前块在参考层中的宏块
  ...
  Int     iBaseMbIdx    = iMbOffset + iBaseMbY * iMbStride + iBaseMbX;           // 对应宏块ID
  MbData& rcBaseMb      = m_rcMbDataCtrlBase.getMbDataByIndex( (UInt)iBaseMbIdx );

  //===== non-Mbaff to non-Mbaff resampling =====     返回的参数
...
MotionUpsampling::xSetPartIdcArray()
{
  ...
  //===== determine all 16 initial partition indexes =====
  {
    ...
    {
      RNOK( xGetRefLayerPartIdc( ( iX << 2 ) + 1, ( iY << 2 ) + 1, m_aaiPartIdc[iX][iY] ) );   // m_aaiPartIdc保存帧内按4x4块顺序的id
...
MotionUpsampling::xSetPredMbData()
{
  ...
  if( m_bInCropWindow )
  {
    rcMbData.setMbMode( m_eMbMode );           // 拷贝MbMode
    rcMbData.setFwdBwd( (UShort)m_uiFwdBwd );
    for( Int iBlk = 0; iBlk < 4; iBlk++ )
    {
      rcMbData.setBlkMode   ( Par8x8( iBlk ), m_aeBlkMode[iBlk] );        // 拷贝宏块分割方式
  ...
  if( ( m_bSCoeffPred || m_bTCoeffPred ) && m_bInCropWindow )
  {
    ...
    rcMbData.copyTCoeffs    ( rcMbDataBase );     //拷贝BaseLayer的变换系数
                              ...
                              MbDataCtrl*     pcMbDataCtrl )
{
  ...
  m_ucLastMbQp      = rcSH.getSliceQp();                //上一个MB的QP
...
ErrVal MbDataCtrl::initMb( MbDataAccess*& rpcMbDataAccess, UInt uiMbY, UInt uiMbX, const Int iForceQp )
{
  ...
  UInt     uiMbYComp    = ( bMbAff ? ( bTopMb ? uiMbY+1 : uiMbY-1 ) : uiMbY );     //就是 uiMbY
  UInt     uiCurrIdx    = uiMbY * m_uiMbStride + uiMbX + m_uiMbOffset;                    //当前宏块序号
  UInt     uiCompIdx    = uiMbYComp    * m_uiMbStride + uiMbX + m_uiMbOffset;    //就是 uiCurrIdx   没有场的情况其实是废的
  ...
  UInt     uiIdxColTop;
  UInt     uiIdxColBot;
  if( ! m_pcSliceHeader->getFieldPicFlag() )                //FRAME的情况 
  {
    UInt  uiMbYColTop = 2 * ( uiMbY / 2 );
    uiIdxColTop       = uiMbYColTop * m_uiMbStride + uiMbX + m_uiMbOffset;               //Colocated Top
    uiIdxColBot       = uiIdxColTop + m_uiMbStride;                                                                //Colocated Bot
  ...
  if( m_pcMbDataAccess )             //第一次为NULL  以后每次都是上一次编码的宏块
  {
  ...
  if( PARSE_PROCESS == m_eProcessingState || ENCODE_PROCESS == m_eProcessingState)
  {
    if( 0 == uiSliceId )
    {
      ...
      rcMbDataCurr.getMbTCoeffs().clear();                                 //由上一个 MB 的QP初始化这个 MB
  ...
  Int icurrSliceGroupID = getSliceGroupIDofMb( uiMapUnit );    // 0  1  2...
  ...
  m_pcMbDataAccess = new (m_pcMbDataAccess) MbDataAccess( rcMbDataCurr,                                      // current
                                                         ...
                                                         *m_pcSliceHeader,
                                                          ...
                                                          m_ucLastMbQp, m_ucLastMbQp4LF );        //  这里设置了 AvailableMask

  ROT( NULL == m_pcMbDataAccess );

  rpcMbDataAccess = m_pcMbDataAccess;      //传递给写入的参数
