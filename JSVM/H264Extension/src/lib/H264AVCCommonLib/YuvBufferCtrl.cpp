...
{
  ROF( m_bInitDone );
  ruiChromaSize = m_uiChromaSize;    //初始化时计算得到的
  return Err::m_nOK;
}
...
ErrVal YuvBufferCtrl::initMb( UInt uiMbY, UInt uiMbX, Bool bMbAff )
{
  ...
  UInt uiYPosFld  = (uiMbY<<3) << m_iResolution;       //不是field   uiYPosFld = uiYPos    是field   uiYPosFld = uiYPos/2
  Int  iStride    = m_acBufferParam[FRAME].m_iStride;
  // chroma
  m_acBufferParam[FRAME]    .m_uiCbOffset = m_uiCbBaseOffset + uiXPos + uiYPos    * iStride / 2;     //这里stride是因为宽度只有一半
