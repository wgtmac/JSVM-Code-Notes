...
ErrVal MotionCompensation::initMb( UInt uiMbY, UInt uiMbX, MbDataAccess& rcMbDataAccess )// TMM_INTERLACE
{
  ...
  if( rcMbDataAccess.getMbData().getFieldFlag() )      //非场  忽略
  {
    uiMbInFrameY >>= 1;
    uiMbY        >>= 1;
  }
  ...
  //  pcRecBuffer =  传入的 rcYuvMbBuffer  是空的 最终用来写入重建值  这里写入预测值
Void MotionCompensation::xPredLuma( YuvMbBuffer* pcRecBuffer, Int iSizeX, Int iSizeY, MC8x8& rcMc8x8D )
{          
  YuvMbBuffer* apcTarBuffer[2];    // 重建的目标   =  pcRecBuffer  
  m_pcSampleWeighting->getTargetBuffers( apcTarBuffer, pcRecBuffer, rcMc8x8D.m_apcPW[LIST_0], rcMc8x8D.m_apcPW[LIST_1] );

  for( Int n = 0; n < 2; n++ )
  {
    YuvPicBuffer* pcRefBuffer = rcMc8x8D.m_apcRefBuffer[n];     //获得参考图像
    if( NULL != pcRefBuffer )
    {
      const Mv cMv = rcMc8x8D.m_aacMv[n][0];   // 获得MV
	  //由MVP(cMv)           从参考图像(pcRefBuffer)中读出对应宏块的值，存入apcTarBuffer[n]
      m_pcQuarterPelFilter->predBlk( apcTarBuffer[n], pcRefBuffer, rcMc8x8D.m_cIdx, cMv, iSizeY, iSizeX );      // 此函数对1/4像素的MV进行选择性扩展
    }
  }                    //   进行加权预测 （就是用权值去修饰一下，两个LIST最终还是输出一个预测图像---pcRecBuffer）
...
ErrVal MotionCompensation::compensateMb( MbDataAccess&    rcMbDataAccess,
                                         RefFrameList&    rcRefFrameList0,
                                         RefFrameList&    rcRefFrameList1,
                                         YuvMbBuffer*  pcRecBuffer,                // 传入的 rcYuvMbBuffer  是空的 最终用来写入重建值  这里应该是写入预测值
                                         Bool             bCalcMv  )
{
  MbMode eMbMode  = rcMbDataAccess.getMbData().getMbMode();
  ...
  case MODE_16x16:
    {
      ...
      Frame* pcRefFrame0 = ( iRefIdx0 > 0 ? rcRefFrameList0[ iRefIdx0 ] : NULL );
      Frame* pcRefFrame1 = ( iRefIdx1 > 0 ? rcRefFrameList1[ iRefIdx1 ] : NULL );
      xGetMbPredData( rcMbDataAccess, pcRefFrame0, pcRefFrame1, cMC8x8D );   //获取预测所用的 mv、 ref_idx、 mvd 和参考帧buffer， 存入 cMC8x8D
      xPredLuma(   pcRecBuffer, 16, 16, cMC8x8D );             // 得到预测的luma宏块
      xPredChroma( pcRecBuffer,  8,  8, cMC8x8D );            // 得到预测的chroma宏块
    }
    break;

  case MODE_16x8:                  // 与上面的不同就是分两个part预测
    {
      if( bCalcMv )
      {
  ...
  case MODE_8x16:                  // 与上面的不同就是分两个part预测
    {
      if( bCalcMv )
      {
...
//  TMM_EC }}
        ...
        Int       iRefIdx0    = rcMbDataAccess.getMbMotionData( LIST_0 ).getRefIdx( PART_8x16_0 );     // getRefIdx( ParIdx8x16 eParIdx  )   没有16x16的声明
        Int       iRefIdx1    = rcMbDataAccess.getMbMotionData( LIST_1 ).getRefIdx( PART_8x16_0 );
        Frame* pcRefFrame0 = ( iRefIdx0 > 0 ? rcRefFrameList0[ iRefIdx0 ] : NULL );
        Frame* pcRefFrame1 = ( iRefIdx1 > 0 ? rcRefFrameList1[ iRefIdx1 ] : NULL );

        xGetMbPredData( rcMbDataAccess, pcRefFrame0, pcRefFrame1, cMC8x8D );     //获取预测所用的 mv ref_idx mvd ， 存入 cMC8x8D==========SKIP模式mv直接由周围宏块测得，ref_idx = 1

        xPredLuma(   pcRecBuffer, 16, 16, cMC8x8D );            // 预测出Luma宏块   保存在 pcRecBuffer
        xPredChroma( pcRecBuffer,  8,  8, cMC8x8D );            // 预测出chroma宏块
        return Err::m_nOK;
      }
    }
    break;

  case MODE_8x8:                          // 划分成8x8的宏块由每个子宏块单独compensateSubMb
  case MODE_8x8ref0:
    printf("function not defined for the case\n");
    RERR();
    break;
...
__inline Void MotionCompensation::xGetMbPredData(       MbDataAccess& rcMbDataAccess,
                                                  ...
                                                  const Frame*     pcRefFrame1,
                                                        MC8x8&    rcMC8x8D )
{                                                                                                 //获得预测所需的 mv  mvd  ref_idx
  rcMC8x8D.clear();                // 先清零（cIdx还在，标志这个8x8块是哪块）  再写入 需要的 MV ref_idx  MVD 和 参考图像的Buffer
  ...
  for (Int n = 0; n < 2; n++)
  {
    const Frame* pcRefFrame = n ? pcRefFrame1 : pcRefFrame0;             //参考帧
    ListIdx eLstIdx            = ListIdx(n);                 //  LIST0   LIST1

    const MbMvData&     rcMbMvdData = rcMbDataAccess.getMbMvdData( eLstIdx );                 //  提取出MVD 供后面第3句写入 rcMC8x8D
    Mv3D& rcMv3D               = rcMC8x8D.m_aacMv[eLstIdx][0];       // 只是引用  还没有值，供下面那一句写入MV和ref_idx
    rcMbDataAccess.getMbMotionData( eLstIdx ).getMv3D( rcMv3D, rcMC8x8D.m_cIdx );        // 《1》  Mv和对应ref_idx 写入 rcMC8x8D

    rcMC8x8D.m_aacMvd[eLstIdx][SPART_4x4_0] = rcMbMvdData.getMv( rcMC8x8D.m_cIdx + SPART_4x4_0 );        //《2》写入MVD
	//至此  只设置了 8x8块第一个 4x4块的 mv、ref_idx 和 mvd

    if( pcRefFrame != NULL )
    {
      iPredCount++;
      rcMv3D.limitComponents( m_cMin, m_cMax );   //防止 mv 越界
	  rcMC8x8D.m_sChromaOffset[eLstIdx] = xCorrectChromaMv( rcMbDataAccess, pcRefFrame->getPicType() );     //无视  仅对FIELD
      rcMC8x8D.m_apcRefBuffer [eLstIdx] = const_cast<Frame*>(pcRefFrame)->getFullPelYuvBuffer();          // 《3》 rcMC8x8D 读取 参考帧
  ...
  if( ( 2 == iPredCount ) && ( 2 == rcSH.getPPS().getWeightedBiPredIdc() ) )             // B slice有效  
  {
