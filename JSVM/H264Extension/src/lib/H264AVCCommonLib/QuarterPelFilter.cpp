...
ErrVal QuarterPelFilter::filterFrame( YuvPicBuffer *pcPelBuffer, YuvPicBuffer *pcHalfPelBuffer )
{
  ...
  XPel*   pucSrc      = pcPelBuffer->getMbLumAddr ();              //整像素图像亮度地址
  ...
  Int     iTmpXSize  = (iMargin + iWidth + iMargin) * 2;                //half-pel后的宽度
  Int     iTmpYSize  = iMargin + iHeight + iMargin;                       //仍然是int-pel的高度 且上下各32没用
  Int     iMarginNew  = iMargin-4;
  Int     x, y;

  XXPel*  psTemp      = new XXPel[iTmpXSize * iTmpYSize];                  //仅仅中间变量
  ::memset( psTemp, 0x00, (iTmpXSize * iTmpYSize)*sizeof(XXPel) );

  ROT( NULL == psTemp )

  XXPel*  ps          = &psTemp[ iMargin * iTmpXSize + 2*iMargin ]; // fix provided by Shijun Sun    luma开始地址
  ...
  {
    ::memcpy( &ps[y*iTmpXSize], &ps[(y-1)*iTmpXSize], iTmpXSize*sizeof(XXPel) );     //逐行拷贝
  }
  ...
  //至此完成了宽度halfpel的插值，仅仅最外面边缘4像素未赋值
  ps = &psTemp[ 4*iTmpXSize + 2*iMargin ]; // fix provided by Shijun Sun    高度上也留了4像素宽度  注意这是中间的过渡变量
  iStride = iTmpXSize;

  Int   iDesStrideHP  = pcHalfPelBuffer->getLStride();
  XPel* pucDesHP      = pcHalfPelBuffer->getMbLumAddr();               //  1/2像素图像亮度地址
  pucDesHP -= (iMarginNew*iDesStrideHP)<<1;                                  //  上面空了8行
  ...
  for( y = -iMarginNew; y < iHeight+iMarginNew; y++ )
  {
    for( x = -2*iMarginNew; x < 2*(iWidth+iMarginNew); x++ )
    {
      ...
      pucDesHP[x+iDesStrideHP] = gClip( ( iTemp + 512) / 1024);         //下一行
    }
    pucDesHP += iDesStrideHP<<1;
    ps     += iStride;
  }
  //实际边缘只有   宽（Margin-4）×2     高  （Margin-4 ）×2    有数据
  delete [] psTemp;
  psTemp = NULL;
  return Err::m_nOK;
}
...
{      //由MVP 从参考图像中读出对应宏块的值，存入pcDesBuffer
  ...
  Int iOffset     = (cMv.getHor() >> 2) + (cMv.getVer() >> 2) * iSrcStride;       //变成 int-pel

  pucSrc += iOffset;      //找到宏块对应位置
