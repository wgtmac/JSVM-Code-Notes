...
YuvPicBuffer::init( XPel*& rpucYuvBuffer )
{
  ...
  if( NULL == rpucYuvBuffer )
  {
    m_pucOwnYuvBuffer = new XPel[ 6 * uiSize ];
	  ::memset( m_pucOwnYuvBuffer, 0x00, (6 * uiSize)*sizeof(XPel) );                //初始化和开辟空间  XPel*& rpucYuvBuffer
  ...
  m_pucYuvBuffer[(6 * uiSize)-1] = 0xde;               //11101111

  m_rcYuvBufferCtrl.initMb();                     //这是 Frame 里面的 YuvBuffer
...
YuvPicBuffer::loadFromPicBuffer( PicBuffer* pcPicBuffer )
{
  ...
  UInt uiSize;
  RNOK( m_rcYuvBufferCtrl.getChromaSize( uiSize ) );
  uiSize *= 6;      //包含luma和chroma以及margin
...
ErrVal YuvPicBuffer::fillMargin()               //进行边缘填充
{
  m_rcYuvBufferCtrl.initMb();
