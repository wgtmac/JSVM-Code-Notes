...
{
	//计算中每个dependency层所有的SNR层vector个数，必须一共16个
  UInt uiVectPos = 0;
  UInt uiQLs;
  for( uiQLs = 0; uiVectPos != 16; uiQLs++ )
  {
    uiVectPos += getMGSVect( uiQLs );    //这个函数如果MGSmode=1，返回每个MGS的vector个数，否则返回16 0 0 0……
    ROTREPORT( uiQLs == 15 && uiVectPos != 16, "MGS vectors don't add up to 16" );
  }
  return uiQLs;    //返回的是CGS中MGS层数
}
...
ErrVal CodingParameter::check()
{
  ...
  setDecompositionStages( uiDecStages );						// 一开始，时间层的分层是由GOP决定的
  ...
  UInt    uiMaxFrameDelay = (UInt)floor( dMaxFrameDelay );				// 可以容忍的最大延时
  Double  dMinUnrstrDelay = Double( ( 1 << m_uiDecompositionStages ) - 1 ) / m_dMaximumFrameRate * 1000.0;	//分层结构导致的最大延时（ms）

  for( UInt uiLayer = 0; uiLayer < getNumberOfLayers(); uiLayer++ )		// 对每一层进行配置
  {
    ...
    UInt              uiLogFactorInOutRate  = getLogFactor( pcLayer->getOutputFrameRate (), pcLayer->getInputFrameRate() );		// 码率输入输出比
    UInt              uiLogFactorMaxInRate  = getLogFactor( pcLayer->getInputFrameRate  (), getMaximumFrameRate       () );		// 对原始文件的采样比
    ...
    if( pcLayer->getUseLongTerm() && m_dMaximumDelay < dMinUnrstrDelay )			// 分层结构的延时超过了要求的延时，开启长期参考帧容易出错
    {
    ...
    pcLayer->setNotCodedStages      ( uiLogFactorInOutRate );		// 输入输出码率比
    pcLayer->setTemporalResolution  ( uiLogFactorMaxInRate );		// 原始文件采样比
    pcLayer->setDecompositionStages ( getDecompositionStages() - uiLogFactorMaxInRate );			// 真正的时间层数
    pcLayer->setFrameDelay          ( uiMaxFrameDelay );
    if( ( uiMaxFrameDelay >> ( uiLogFactorMaxInRate + uiLogFactorInOutRate ) ) == 0 || ( getDecompositionStages() - uiLogFactorMaxInRate - uiLogFactorInOutRate ) == 0 )		// BUG found by wgtmac：getDecompositionStages()已经减去过uiLogFactorMaxInRate（不编码层数）
    {			// 原始视频可以容忍的延时都被采样去掉了  或者  时间分层都不编码 ===》不能分层了
      pcLayer->setPicCodingType( 1 );			// PicCodingType=1 不采用时间分层
    }
    ...
    if( pcBaseLayer )		// 检查该层参考的那一层
    {
      ...
      ROTREPORT( pcLayer->getInputFrameRate() < pcBaseLayer->getInputFrameRate(), "Input frame rate less than base layer output frame rate" );		// BUG？
      ...
      pcBaseLayer->setConstrainedIntraPred( 1 );  //mark

      if( pcLayer->getSliceSkip() )
      {
        pcLayer->setSliceSkipTLevelStart( pcLayer->getSliceSkip() - 1 );				// 技术上使用，为了后面判断是不是SliceSkip
        pcLayer->setConstrainedIntraPred( 1 ); 
      }
    ...
    if( pcLayer->getTCoeffLevelPredictionFlag() )
    {
      pcLayer->setConstrainedIntraPred( 1 );      //mark
...
LayerParameters::setAndCheckProfile( CodingParameter* pcCodingParameter )
{
  ...
  for( UInt uiTestLayer = m_uiLayerId + 1; uiTestLayer < pcCodingParameter->getNumberOfLayers(); uiTestLayer++ )		// 只对该层以上的增强层
  {
  ...
  m_uiNumDependentDId     = 1;			// 增强层，默认为D_id+1或者是第一个新的spatial层
  if( m_uiBaseLayerId != MSYS_UINT_MAX )			// 如果跳过这个函数，表示该spatial层不参考底层
  {
    if( pcCodingParameter->getLayerParameters( m_uiBaseLayerId ).m_uiLayerCGSSNR == m_uiLayerCGSSNR )
    {
      m_uiNumDependentDId = 0;				// 连续的spatial层，不改变D_id
    }
    m_uiNumDependentDId  += pcCodingParameter->getLayerParameters( m_uiBaseLayerId ).m_uiNumDependentDId;		// spatial的时候，D_id+1
  ...
  {					// q_id>0
    ...
    {
      SETREPORT( m_uiPicCodingType, 1, "B slices disabled for MGS enhancement" );			// CGS和MGS的增强层必须与其BL相同（不设立B帧）
    }                                              
    if( m_uiPicCodingType == 0 && pcCodingParameter->getLayerParameters( m_uiBaseLayerId ).getPicCodingType() != 0 )
    {
      m_uiPicCodingType = 2;     //其他情况非零则为2
    }
  }
  //----- update IDR period -----
  if( bIntraOnly )
...
LayerParameters::updateWithLevel( CodingParameter* pcCodingParameter, UInt& ruiLevelIdc )
{
  ...
  if( m_uiBaseLayerId != MSYS_UINT_MAX )
  {
    ...
    ROF( uiBaseLevel );
    ruiLevelIdc       = getMaxLevel( ruiLevelIdc, uiBaseLevel );            //增强层的level不能比基本层的小
  }
  ...
  m_uiLevelIdc = ruiLevelIdc;             //根据Profile修改level后最终得到level
  return Err::m_nOK;
}
...
LayerParameters::xIsIntraOnly( CodingParameter* pcCodingParameter )
{
  ...
  ROFRS( m_uiDecompositionStages        == m_uiNotCodedStages,                  false );		// true只有一层TL，就是每层就一帧
  ROFRS( pcCodingParameter->m_uiGOPSize == pcCodingParameter->m_uiIntraPeriod ||
         pcCodingParameter->m_uiGOPSize == (UInt)m_iIDRPeriod                 ||
         pcCodingParameter->m_uiGOPSize == (UInt)m_iLayerIntraPeriod,           false );	//周期相同，那么这一帧就是I/IDR
  ROTRS( m_uiBaseLayerId                == MSYS_UINT_MAX,                       true  );
  return pcCodingParameter->getLayerParameters( m_uiBaseLayerId ).isIntraOnly();		// 还要看BL是不是全I帧
}


Bool
LayerParameters::xHasRestrictedESS( CodingParameter* pcCodingParameter )		// 严格限制了RefLayer的Scale比例
{
  ROTRS( m_uiBaseLayerId                            == MSYS_UINT_MAX, true  );
...
LayerParameters::xForceScalableBaselineProfile( CodingParameter* pcCodingParameter )
{
  ...
  ROTRS( m_uiBaseLayerId == MSYS_UINT_MAX,  Err::m_nOK  );				// 严格限制了RefLayer的Scale比例
