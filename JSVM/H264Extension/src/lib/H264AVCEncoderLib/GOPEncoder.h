...
public:
  ...
  AccessUnitData& getAccessUnitData( UInt uiAUIndex )
  {
    ...
    for( ; iter != end; iter++ )
    {
      ...
      {
        return (*iter);              //找到对应序号的AccessUnit
      }
    }
    AccessUnitData cAUData( uiAUIndex );              //没找到 新建一个空的放到list尾部
...
protected:
  //----- instances -----
  ExtBinDataAccessor            m_cExtBinDataAccessor;              //用来操纵比特流
  BinData                       m_cBinData;
  ...
  UInt                          m_uiNotCodedStages;                   // number of stages that are only used for temporal downsampling    丢弃几个时域层
  ...
  Bool                          m_bH264AVCCompatible;                 // H.264/AVC compatibility                      ***The lowest DependencyLayer,  that is to say DependencyId = 0
  Bool                          m_bInterLayerPrediction;              // inter-layer prediction                     nal_unit_header_svc_extension( )中的 no_inter_layer_pred_flag
  ...
  //----- frame memories -----
  Frame*                        m_apcFrameTemp[NUM_TMP_FRAMES];       // auxiliary frame memories 0和1是辅助BL上采样的   0也可能是spatial
...
//TMM_WP
  ...
  UInt                          m_uiNewlyCodedBits;    //保存每一个layer编码的比特数
...
//JVT-T054}
//DS_FIX_FT_09_2007
  Bool                          m_bDiscardable;                 //nal_unit_header_svc_extension( )中的 discardable_flag
//~DS_FIX_FT_09_2007
// JVT-U085 LMI
...
// JVT-U116 W062 LMI
  ...
  Bool                          m_bGOPInitialized;                          //GOP是否被初始化完毕
  MbDataCtrl*                   m_pcBaseMbDataCtrl;

  UInt                          m_uiEncodeKeyPictures;                     //编码KeyPicture的模式
  Bool                          m_bSameResBL;
  Bool                          m_bSameResEL;
  Bool                          m_bMGS;
  UInt                          m_uiMGSKeyPictureControl;
  Bool                          m_bHighestMGSLayer;
  Bool                          m_abCoded[(1<<MAX_DSTAGES)+1];                //每帧是否已经编码  猜测
  ...
  UInt          m_uiFrameIdWithSmallestTId;                                                                  //辅助变量  得到最小时域层的FrameID(播放顺序)
  UInt          m_uiNumFramesLeftInGOP;                                                                      //GOP中还没有编码的帧数
  UInt          m_auiPicCodingType            [(1<<MAX_DSTAGES)+1];                        //编码类型 ---FRAME
  UInt          m_auiFrameIdToTemporalId      [(1<<MAX_DSTAGES)+1];                  //由FrameID得到GOP时域分层层号的数组
  UInt          m_auiCodingIndexToFrameId     [(1<<MAX_DSTAGES)+1];                 //GOP中由编码顺序得到播放顺序
  UInt          m_auiFrameIdToFrameNumOrLTIdx [(1<<MAX_DSTAGES)+1];          // 由播放顺序得到该帧的FrameNum 
  UIntList      m_acRefPicListFrameId_L0      [(1<<MAX_DSTAGES)+1];                   //按帧播放顺序存放的参考队列  LIST0
  UIntList      m_acRefPicListFrameId_L1      [(1<<MAX_DSTAGES)+1];                   //LIST1
  Bool          m_abMaxLongTermIdxSend        [    MAX_DSTAGES +1];
  UIntList      m_acActiveRefListFrameNum     [    MAX_DSTAGES +1];                  //每个时间层里面有效的作为参考的FrameNum
