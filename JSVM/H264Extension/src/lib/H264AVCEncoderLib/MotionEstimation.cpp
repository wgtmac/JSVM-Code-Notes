                               ...
                               SampleWeighting*  pcSampleWeighting )
{
  ...
  RNOK( MotionEstimationCost::xInit( (m_cParams.getSearchRange() << 2), pcRateDistortionIf ) );		// 初始化mv每个分量每个pos的cost
  ...
  m_uiSpiralSearchEntries *= m_uiSpiralSearchEntries;			// MV搜索的框大小（int）
  ...
  for( Int offset = 1; offset <= iRange; offset++ )			// 按中心旋转顺序初始化m_pcMvSpiralSearch[]
  {
    Int l;
...
ErrVal
MotionEstimation::estimateBlockWithStart( const MbDataAccess&          rcMbDataAccess,         // 当前块内容
                                          const Frame&                 rcRefFrame,                       // 参考帧
                                          Mv&                          rcMv, // start and result           开始估计的mv  （附近块得到的mvp，或者BL的mv）
                                          const Mv&                    rcMvPred,                             // 由附近块预测出的mvp                跳出误区：MVP是用来被mv减去得到mvd的，不一定是搜索起点
                                          UInt&                        ruiBits,
                                          UInt&                        ruiCost,
                                          const UInt                   uiBlk,                                         //起始的4x4块 id
                                          const UInt                   uiMode,
                                          ...
                                          const MEBiSearchParameters*  pcBSP )
{
  ...
  YuvMbBuffer*  pcWeightedYuvBuffer  = 0;      //由原始数据 进行or不进行 加权得到的结果 （有加权为cWeightedYuvBuffer，无加权为 OrgData）
  YuvPicBuffer* pcRefPelData[2];          //参考帧
  YuvMbBuffer   cWeightedYuvBuffer;                  //有加权时 地址给  pcWeightedYuvBuffer

  pcRefPelData[0] = const_cast<Frame&>(rcRefFrame).getFullPelYuvBuffer();    //int-pel 参考帧
  pcRefPelData[1] = const_cast<Frame&>(rcRefFrame).getHalfPelYuvBuffer();   //half-pel 参考帧

  m_pcXDistortion->set4x4Block( cIdx );                //  原始帧起点
  pcRefPelData[0]->set4x4Block( cIdx );                 //  int-pel参考帧起点
  pcRefPelData[1]->set4x4Block( cIdx );                 //  half-pel参考帧起点

  UInt   uiMinSAD  = MSYS_UINT_MAX;
  Mv     cMv       = rcMv; // start value   开始时是MVP
  Double fWeight   = 1.0;
  ...
  xSetMEPars      ( 2, ( 1 != m_cParams.getFullPelDFunc() ) );           // 设置ME的参数  SAD还是SSE
  xSetPredictor   ( rcMvPred );
  m_pcXDistortion ->getDistStruct( uiMode, m_cParams.getFullPelDFunc(), false, m_cXDSS );          // 设置计算dist的参数====m_cXDSS
  m_cXDSS.pYOrg   = pcWeightedYuvBuffer->getLumBlk();           // 原始帧
  m_cXDSS.pUOrg   = pcWeightedYuvBuffer->getCbBlk ();
  m_cXDSS.pVOrg   = pcWeightedYuvBuffer->getCrBlk ();
  
  m_acMvCandList.clear();           // 清空 mv的 vector
  ...
  else
  {
    ...
    case FAST_SEARCH:
      ...
      break;
    case TZ_SEARCH:                 // new fast mode
      {
        m_acMvCandList.push_back( rcMvPred );     // 第一个是MVP
        rcMbDataAccess.addMvPredictors( m_acMvCandList );     //  A/B/C块的MV也存进去
        xTZSearch( pcRefPelData[0], cMv, uiMinSAD, m_bELWithBLMv );          // 参数：  整像素参考帧 | 预测得到的MV(输出) | 最小的SAD   
      }
      break;
    default:
      RERR();
  ...
  Short sHor      = cMv.getHor();      //ME得到的 mv
...
ErrVal MotionEstimation::initMb( UInt uiMbPosY, UInt uiMbPosX, MbDataAccess& rcMbDataAccess )    //设置MV的范围
{
...
Void MotionEstimation::xTZSearch( YuvPicBuffer *pcPelData, Mv& rcMv, UInt& ruiSAD, Bool bEL, Int iSearchRange )
{
  ...
  SearchRect cSearchRect;
  rcMv >>= 2;
  cSearchRect.init( iSearchRange, rcMv, MotionCompensation::m_cMin, MotionCompensation::m_cMax );          // 初始化所有区域
