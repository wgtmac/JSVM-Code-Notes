...
ErrVal CabacWriter::startSlice( const SliceHeader& rcSliceHeader )
{
  ...
  RNOK( xInitContextModels( rcSliceHeader ) );               //初始化CABAC上下文  每个slice都要初始化一遍
  RNOK( CabaEncoder::start() );                                            //设置CABAC编码器初始状态
                                   ...
                                   UInt           uiStop )
{
  ...
  ROT( uiStart == uiStop );
  UInt uiNumSig = xGetNumberOfSigCoeff( piCoeff, eResidualMode, pucScan, uiStart, uiStop );     // 找到非零系数个数
