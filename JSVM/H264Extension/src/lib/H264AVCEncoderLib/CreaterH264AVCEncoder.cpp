...
CreaterH264AVCEncoder::writeParameterSets( ExtBinDataAccessor* pcExtBinDataAccessor,
                                           ...
                                           Bool&               rbMoreSets )
{
  if( m_pcCodingParameter->getAVCmode() )           //AVC模式
  {
    RNOK( m_pcPicEncoder->writeAndInitParameterSets( pcExtBinDataAccessor, rbMoreSets ) );    //写SPS和PPS
...
CreaterH264AVCEncoder::init( CodingParameter* pcCodingParameter )
{
  ...
  m_pcCodingParameter = pcCodingParameter;              //传入参数就是m_pcCodingParameter

  RNOK( RateDistortion::create( m_pcRateDistortion ) );       //RateDistortion    率失真
  RNOK( m_pcBitWriteBuffer          ->init() )                            //BitWriteBuffer    比特流缓存               
  RNOK( m_pcBitCounter              ->init() );                             //BitCounter          比特计数器
  RNOK( m_pcXDistortion             ->init() );                             //XDistortion          计算失真函数和参数
  RNOK( m_pcSampleWeighting         ->init() );                      //SampleWeighting  加权函数
 
  //NalUnitEncoder     NAL单元编码
  ...
  //ControlMngH264AVCEncoder          编码器控制管理
  ...
  //Bug_Fix JVT-R057{
  if(m_pcCodingParameter->getLARDOEnable())               //CodingParameter的m_uiLARDOEnable     JVT-R057 LA-RDO 在有error的时候采用的拉格朗日乘子
  {
    Bool bFlag=false;
  for( UInt uiLayer = 0; uiLayer < m_pcCodingParameter->getNumberOfLayers(); uiLayer++ )
  {
