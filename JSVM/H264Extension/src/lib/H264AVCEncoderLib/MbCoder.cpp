...
ErrVal MbCoder::encode( MbDataAccess& rcMbDataAccess,
                        ...
                        Bool          bTerminateSlice,                  // 是否是 slice 的终止
                        Bool          bSendTerminateSlice )        // 是否要终止 slice 
{
  ROF( m_bInitDone );
  ...
  // JVT-W043 {
  if ( bRateControlEnable )            //    RateControl  暂时不看 ******************************START***************************
  {
    pcGenericRC->m_iRCTotalBits = getBitCount();
    pcGenericRC->m_iRCTextureBits = 0;
  }
  // JVT-W043 }                              //    RateControl  暂时不看 *******************************END****************************

  //===== skip flag =====
  Bool  bIsCoded  = ! rcMbDataAccess.isSkippedMb();

  RNOK( m_pcMbSymbolWriteIf->skipFlag( rcMbDataAccess ) );                  // mb_skip_flag

  MbSymbolWriteIf *pcCurrentWriter = m_pcMbSymbolWriteIf;                    //CABAC编码
  UInt uiMGSFragment = 0;
  for( uiMGSFragment = 0;
       rcMbDataAccess.getSH().getSPS().getMGSCoeffStop( uiMGSFragment ) < 16;              // 与MGS增强层有关
       uiMGSFragment++ )
  {
  ...
  if( bIsCoded )
  {
    ...
    {
      RNOK( m_pcMbSymbolWriteIf->fieldFlag( rcMbDataAccess) );              //  mb_field_decoding_flag    FRAME情况就没有
    }
    ...
    if( ! rcMbDataAccess.getMbData().getBLSkipFlag() )
    {
      ...
      RNOK( m_pcMbSymbolWriteIf->mbMode( rcMbDataAccess ) );              // mb_type
    ...
    if( ! rcMbDataAccess.getMbData().getBLSkipFlag() )
    {
      ...
      else if( rcMbDataAccess.getMbData().isIntra() )
      {
        ...
        RNOK( xWriteIntraPredModes( rcMbDataAccess ) );    //transform_size_8x8_flag | prev_intraNxN_pred_mode_flag | rem_intra4NxN_pred_mode | intra_chroma_pred_mode
      }
      else
      {
        //===== MOTION INFORMATION =====
    ...
    else
    {
      for( B8x8Idx c8x8Idx; c8x8Idx.isLegal(); c8x8Idx++ )
      {
        if( (uiCbp >> c8x8Idx.b8x8Index()) & 1 )
        {
          ...
          {
            RNOK( xScanLumaBlock( rcMbDataAccess, rcMbTCoeff, cIdx, uiStart, uiStop ) );             //  8x8块 Cabac 编码
          }
        }
      }
    }
...
ErrVal MbCoder::xScanLumaIntra16x16( MbDataAccess& rcMbDataAccess, const MbTransformCoeffs& rcTCoeff, Bool bAC, UInt uiStart, UInt uiStop )
{
  ...
  {
    RNOK( m_pcMbSymbolWriteIf->residualBlock( rcMbDataAccess, B4x4Idx(0), LUMA_I16_DC, uiStart, uiStop ) );        // DC系数块
  }
  ROFRS( bAC && uiStop > 1, Err::m_nOK );

  for( S4x4Idx cIdx; cIdx.isLegal(); cIdx++)             //  16个4x4AC系数块
  {
