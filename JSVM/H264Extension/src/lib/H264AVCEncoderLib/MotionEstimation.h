...
protected:
  typedef struct
  {
    Void init( Short sLimit, Mv& rcMvPel, Mv cMin, Mv cMax )         // sLimit 为 SearchRange
    {
      cMin >>= 2;
      cMax >>= 2;     // int-pel
      Short sPosV   = cMax.getVer() - rcMvPel.getVer();                      // 正水平方向裕量
      Short sNegV   = rcMvPel.getVer() - cMin.getVer();                      // 负水平方向裕量
      iNegVerLimit  = gMin( sLimit, sNegV) - rcMvPel.getVer();         // 正水平方向最大值
      iPosVerLimit  = gMin( sLimit, sPosV) + rcMvPel.getVer();          // 负水平方向最小值
...
protected:
  ...
  Int                       m_iMaxLogStep;
  Mv*                       m_pcMvSpiralSearch;			// 中心旋转顺序搜索的mv
