...
// JVT-U116 W062 LMI
  ...
  {
    UInt uiLId;
    for( uiLId = m_uiDependencyId + 1; uiLId < pcCodingParameter->getNumberOfLayers() && ! m_bSameResEL; uiLId++ )			// 一开始DependencyId就是Layerid
    {
      LayerParameters& rcEL   = pcCodingParameter->getLayerParameters( uiLId );
      if( rcEL.getBaseLayerId() == m_uiDependencyId )				// 找到参考本层的增强层
      {
        m_bSameResEL          = ( pcLayerParameters->getFrameWidthInSamples () == rcEL.getFrameWidthInSamples () &&
                                  ...
                                  rcEL.getResizeParameters().m_iExtendedSpatialScalability == ESS_NONE );				// 比较两者分辨率
      }
    }
  }
  m_bMGS                      = pcCodingParameter->getCGSSNRRefinement    () == 1 && ( m_bSameResBL || m_bSameResEL );		// 一定要分辨率相同
  ...
  m_bHighestMGSLayer          = m_bMGS && !m_bSameResEL;		// 指示该层是不是最高层的MGS
  m_uiLastCodedFrameIdInGOP   = MSYS_UINT_MAX;
  m_uiLastCodedTemporalId     = MSYS_UINT_MAX;
																				// 输入文件的采样比				最大的延时帧数
  RNOK( xInitCodingOrder( m_uiDecompositionStages, m_uiTemporalResolution, m_uiFrameDelay ) );		// 安排分层B帧的编码顺序
  ...
  m_dBaseQpLambdaMotionLP   = pcLayerParameters->getQpModeDecisionLP          ();			// ME和MD的QP
  m_dBaseQPResidual         = pcLayerParameters->getBaseQpResidual            ();						// 残差用的QP
...
{				//									m_uiDecompositionStages, m_uiTemporalResolution,			m_uiFrameDelay 
  ...
  for( UInt uiTemporalId = 1; uiTemporalId <= uiDecompositionStages; uiTemporalId++ )		// 还是按照采样前的帧数进行时间分层
  {
    UInt uiOffset = ( 1U << ( uiDecompositionStages - uiTemporalId ) );			// 该时间层每帧的间隔
    for( UInt uiFrameIdInGOP = uiOffset; uiFrameIdInGOP <= uiMaxGOPSize; uiFrameIdInGOP += ( uiOffset << 1 ) )
    {
      auiTemporalId[ uiFrameIdInGOP ] = uiTemporalId;				// 安排GOP中每个帧的时间层号
    }
  }
  //===== initialize the array required =====
  for( UInt uiFrameId = 1; uiFrameId < uiMaxGOPSize; uiFrameId++ )			//uiMaxGOPSize没有包括在内，因为它算下一个GOP的开始
  {
    Bool bSet = false;
    for( UInt uiNextId = uiFrameId + 1; !bSet; uiNextId++ )			// 找它后面依赖的那一帧
    {
  ...
  for( UInt uiPeriodOffset = 1; uiPeriodOffset <= uiMaxGOPSize; uiPeriodOffset += uiPeriod )		// uiPeriod是允许delay的周期
  {
  ...
  UInt      uiCodingNumber = 0;				// 计算编码顺序的GOP中帧号
  for( UInt uiFrameIdInGOP = 0; uiFrameIdInGOP <= uiMaxGOPSize; uiFrameIdInGOP++ )    // 从第一帧开始  按照依赖顺序编码最先被依赖的帧
  {
    if( ! abIndexAssigned[ uiFrameIdInGOP ] )			// 该帧未安排顺序
    {
      // get list of required pictures
      UInt auiRequiredList[MAX_DSTAGES];			// 前向时间层的依赖 直到已经编码了的
  ...
  for( ; uiModIdx <= uiMaxGOPSize; uiModIdx++ )
  {
    ...
    if(  uiVirtFrameIdInGOP % uiTempResStep )				// 属于output rate抽取部分的帧
    {
      m_auiCodingIndexToFrameId[ uiModIdx ] = MSYS_UINT_MAX;
    }
    else
    {
      UInt  uiFrameIdInGOP                          = uiVirtFrameIdInGOP >> uiTemporalSubSampling;			// 先排除抽取的帧
      m_auiFrameIdToTemporalId  [ uiFrameIdInGOP ]  = auiTemporalId[ uiVirtFrameIdInGOP ];			// 计算帧号对应的时间层
...
{           //初始化帧的大小   LayerEncoder的YuvBuffer
  RNOK( m_pcYuvFullPelBufferCtrl->initMb() );            //integer-pel
  RNOK( m_pcYuvHalfPelBufferCtrl->initMb() );           //half-pel

  if( ! pcFrame->isHalfPel() )
  {
    XPel* pHPData = NULL;
    RNOK( pcFrame->initHalfPel( pHPData ) );      //与PicBuffer 的 m_pucYuvBuffer = m_pucOwnYuvBuffer关联了  保存1/2像素的图像
  }

  RNOK( pcFrame->extendFrame( m_pcQuarterPelFilter, ePicType, bFrameMbsOnlyFlag ) );           //边缘填充和像素插值
...
LayerEncoder::xInitExtBinDataAccessor( ExtBinDataAccessor& rcExtBinDataAccessor )
{
  ...
  m_cBinData.setMemAccessor ( rcExtBinDataAccessor );       //用 rcExtBinDataAccessor 来操纵 m_pucWriteBuffer （大小m_uiWriteBufferSize）
                                           ...
                                           ExtBinDataAccessor*     pcExtBinDataAccessor )
{
  ...
  UInt    uiNewSize     = pcExtBinDataAccessor->size();     //已写入的数据大小 （几个UChar）
  UChar*  pucNewBuffer  = new UChar [ uiNewSize ];       // 这样空间最省  之前开辟的有多的 浪费了
  ROF( pucNewBuffer );
  memcpy( pucNewBuffer, pcExtBinDataAccessor->data(), uiNewSize * sizeof( UChar ) );      //内容拷贝到临时变量 pucNewBuffer 中  其实是LayerEncoder中的缓存里的

  ExtBinDataAccessor* pcNewExtBinDataAccessor = new ExtBinDataAccessor;
  ROF( pcNewExtBinDataAccessor );

  m_cBinData              .reset          ();                                  //删除之前的  m_cBinData
  m_cBinData              .set            (  pucNewBuffer, uiNewSize );                        //m_cBinData添加为新写入的内容
  m_cBinData              .setMemAccessor ( *pcNewExtBinDataAccessor );        //该数据由新的 *pcNewExtBinDataAccessor  来操纵 并放入队列中 OutPutList
  rcExtBinDataAccessorList.push_back      (  pcNewExtBinDataAccessor );

  m_cBinData              .reset          ();                                  //删除之前的  m_cBinData
  m_cBinData              .setMemAccessor ( *pcExtBinDataAccessor );                //老数据清空  仍由pcExtBinDataAccessor操纵   现在都是空的
...
LayerEncoder::xEncodeLayerRepresentation( ExtBinDataAccessorList& rcOutExtBinDataAccessorList,   // rcAccessUnitData.getNalUnitList()
                                          ...
                                          PicOutputDataList&      rcPicOutputDataList,            // 输出图像list 
                                          UInt                    uiFrameIdInGOP,                                 // GOP内第几帧
                                          PicType                 ePicType )                                         // FRAME
{
  UInt          uiBits        = 0;                //临时记录比特数
  UInt          uiBitsSEI     = 0;              //记录SEI比特数
  SliceHeader*  pcSliceHeader = rcControlData.getSliceHeader( ePicType );
  ROF( pcSliceHeader );
  FMO*          pcFMO         = pcSliceHeader->getFMO();             //获得关于Slice Group的信息
  ROF( pcFMO );

  ExtBinDataAccessorList  cTmpExtBinDataAccessorList;     // 给 closeAndAppendNalUnits 函数用的list  临时List
  ExtBinDataAccessorList  acExtBinDataAccessorList[16];     // 每个 MGSVector层用的 list
  PicOutputDataList       acPicOutputDataList     [16];

  //----- subsequence SEI -----               暂时没看 ********START*******
  ...
  for( Int iSliceGroupID = pcFMO->getFirstSliceGroupId(); ! pcFMO->SliceGroupCompletelyCoded( iSliceGroupID ); iSliceGroupID = pcFMO->getNextSliceGroupId( iSliceGroupID ) )
  {
		...
		rcControlData.m_uiCurrentFirstMB    = pcFMO->getFirstMacroblockInSlice( iSliceGroupID );    //获得当前SliceGroup第一个MB

    //===== loop over slices (inside slice group) =====
		while( !rcControlData.m_bSliceGroupAllCoded )           //当还未编码完SliceGroup
		{
			//----- set slice parameters -----     设置slice group的起止宏块
			rcControlData.m_bSliceGroupAllCoded = ( m_uiSliceMode == 0 );      //JVT-X046   encoder.cfg   0：不分slice
			...
			//----- write prefix NAL unit -----     可见 prefix NalUnit后 一定接着 type = 1/5的
			...
			{
				RNOK( xWritePrefixUnit( acExtBinDataAccessorList[0], *pcSliceHeader, uiBits ) );       //写nal_unit_type=20的prefix NalUnit
			}

			//----- init slice data NAL UNIT -----    同PrefixUnit一样初始化
			...
			RNOK( m_pcNalUnitEncoder->write( *pcSliceHeader ) );               //真正的写 SliceHeader
			...
			//----- close NAL UNIT -----
			UInt auiBits[16];     // 上面的 encodeSliceSVC() 函数已经进行了cabac熵编码 
			RNOK( m_pcNalUnitEncoder->closeAndAppendNalUnits( auiBits,
                                                                  cTmpExtBinDataAccessorList,            // 临时NalUnit的 BinDataAccessorList
                                                                  &m_cExtBinDataAccessor,                 //LayerEncoder 的 BinDataAccessor
                                                                  m_cBinData,
                                                                  m_pcH264AVCEncoder,
			...
			{
				acExtBinDataAccessorList[iMGSIdx].push_back( cTmpExtBinDataAccessorList.popFront() );    // 从临时的list中读取每个MGS层的数据分别存入
  ...
  {
    rcOutExtBinDataAccessorList += acExtBinDataAccessorList [uiMGSIdx];       //list合并
...
ErrVal                      //question ：  m_uiLowPassIntraPeriod 应该初始化为 MSYS_UINT_MAX 么???????
...
{                                             //传入参数：    帧播放顺序     帧的时域分层号
  ...
  ROF( m_acRefPicListFrameId_L0[ uiFrameIdInGOP ].empty() );           //现在每帧的参考队列
  ROF( m_acRefPicListFrameId_L1[ uiFrameIdInGOP ].empty() );

  //===== check for intra =====
  Bool bIntra  = ( uiFrameIdInGOP == 0 );           //GOP里面播放顺序为0的是Intra  
  if( !bIntra && ( m_uiLowPassIntraPeriod != MSYS_UINT_MAX || m_iIDRPeriod != 0 || m_iLayerIntraPeriod != 0 ) )             //没看懂 ?
  {                                                                                                    //   一般为GOPSIZE                  一般为GOPSIZE
    UInt  uiCurrFrame     = (   m_uiGOPNumber                 << m_uiDecompositionStages ) + uiFrameIdInGOP;       //该帧是序列里的第几帧
  ...
  ROT( bIntra && uiTemporalId > 0 );             //时域大于0的层不能为intra预测

  //===== set backward and forward list =====
  UInt      uiMaxTId  = ( uiTemporalId ? uiTemporalId - 1 : uiTemporalId );           //当前帧时域层的下面一层（除了0层 没有参考过同层的）
  UIntList  cFwdList;              //暂时存参考图像的 播放顺序id-------> frameId
  UIntList  cBwdList;
  if( !bIntra )
  {
    for( Int iBwdFrameId = (Int)uiFrameIdInGOP - 1; iBwdFrameId >= 0 && cBwdList.size() < m_uiMaxNumRefFrames; iBwdFrameId-- )   //搜索GOP中的播放顺序在前的帧 （时间层在当前帧下面的）
    {
    ...
    for( Int iFwdFrameId = (Int)uiFrameIdInGOP + 1; iFwdFrameId <= (Int)m_uiGOPSize && cFwdList.size() < m_uiMaxNumRefFrames; iFwdFrameId++ )  //搜索GOP中的播放顺序在后的帧 （时间层在当前帧下面的）
    {
    ...
    ROT( cBwdList.empty() && cFwdList.empty() );    //不可能为空  因为至少会加入首尾两个
  }
  if( m_uiPicCodingType == 0 && cBwdList.empty() )     //PicCodingType = 0时 BwdList不能空
  {
    while( cFwdList.size() )
    {
  ...
  m_auiPicCodingType    [ uiFrameIdInGOP ]  = m_uiPicCodingType;           //设置编码类型
  if( m_auiPicCodingType[ uiFrameIdInGOP ] == 2 && cBwdList.size() && cFwdList.size() && m_uiBaseLayerId != MSYS_UINT_MAX )   //说明只对空间增强层才有用
  {
    UInt uiShift             = gMin( m_uiDecompositionStages, m_uiDecompositionStages + 1 - uiTemporalId );
    UInt uiFrmIdInTLayer     = uiFrameIdInGOP >> uiShift;                       //计算该帧在该时域层属于第几帧（播放顺序）
    UInt uiBasePicCodingType = m_pcH264AVCEncoder->getPicCodingType( m_uiBaseLayerId, uiTemporalId, uiFrmIdInTLayer );   //得到其空间参考层的PicCodingType
  ...
  //===== set reference list sizes =====    设置参考队列长度
  UInt  uiL0Size  = (UInt)cBwdList.size();
  UInt  uiL1Size  = (UInt)cFwdList.size();
  if( m_auiPicCodingType[ uiFrameIdInGOP ] == 1 )             //只有L0
  {
    uiL0Size += uiL1Size;
    uiL1Size  = 0;
  }
  else if( m_auiPicCodingType[ uiFrameIdInGOP ] == 2 && uiL1Size )    //L0和L1
  {
    uiL0Size += uiL1Size;
    uiL1Size  = uiL0Size;
  }

  //===== set prediction lists =====    从该函数内的UIntList  cFwdList和cBwdList中将参考图像存入LayerEncoder中的m_acRefPicListFrameId_L0/1中
  UIntList& rcList0 = m_acRefPicListFrameId_L0[ uiFrameIdInGOP ];             //取得refList
  ...
  else // ( m_uiPicCodingMode == 1 || m_uiPicCodingMode == 2 )
  {
    ...
    {
      if( m_auiFrameIdToTemporalId[*cFIter] < m_auiFrameIdToTemporalId[*cBIter] )    //list0先存时间层低的   离该帧播放顺序越近的越先存
      {
        rcList0.push_back   ( *cFIter );
        rcList0.push_back   ( *cBIter );
        if( m_auiPicCodingType[ uiFrameIdInGOP ] == 2 )                      //list1先存时间层高的   离该帧播放顺序越近的越先存
        {
    ...
    for( ; cBIter != cBwdList.end(); cBIter++ )                   //剩余的存入
    {
      rcList0.push_back   ( *cBIter );
  ...
  //std::cout << "FrameIdInGOP : " << (int)uiFrameIdInGOP << "  TemporalID : " << (int)uiTemporalId ;; //<< std::endl ;
  //std::cout << " List 0 : " ;
  //for( UIntList::iterator cL0Iter = rcList0.begin() ; cL0Iter != rcList0.end() ; cL0Iter++ )
  //{
	 // std::cout << (int)*cL0Iter << " " ;
  //}
  ////std::cout << std::endl ;
  //std::cout << "\tList 1 : " ;
  //for( UIntList::iterator cL1Iter = rcList1.begin();  cL1Iter != rcList1.end() ; cL1Iter++ )
  //{
	 // std::cout << (int)*cL1Iter << " " ;
  //}
  //std::cout << std::endl ;
...
ErrVal
LayerEncoder::xSetReorderingCommandsST( RefPicListReOrdering& rcReOrdering,           //对RefList中的所有短期参考帧进行排序
...
LayerEncoder::xSetSliceTypeAndRefLists( SliceHeader& rcSliceHeader, UInt uiFrameIdInGOP, PicType ePicType )
{
  ...
  UInt      uiCurrFrameNum  = ( m_bUseLongTermPics ? rcSliceHeader.getLongTermFrameIdx() : rcSliceHeader.getFrameNum() );     //LongTerm参考没看
  UInt      uiCurrPicNum    = ( bFieldPic ? 2 * uiCurrFrameNum + 1 : uiCurrFrameNum );
  UIntList& rcFrameIdList0  = m_acRefPicListFrameId_L0[ uiFrameIdInGOP ];             //这个list存放的是FrameId
  ...
  if( b2ndField )                 //与场有关 不看之
  {
    ROT( uiCurrFrameNum == MSYS_UINT_MAX );
    cPicNumList0.push_back( uiCurrPicNum - 1 );
  }
  for( UIntList::iterator iter0 = rcFrameIdList0.begin(); iter0 != rcFrameIdList0.end(); iter0++ )         //把list由FrameId转到FrameNum
  {
    UInt uiFrameNum =  m_auiFrameIdToFrameNumOrLTIdx[ *iter0 ];
    ROT( uiFrameNum == MSYS_UINT_MAX );                 //在参考队列里面的帧 应该都已经在m_auiFrameIdToFrameNumOrLTIdx[]中设置好了FrameNum
    if ( bFieldPic )
    {
  ...
  //===== set slice type ====
  SliceType eSliceType    = ( cPicNumList0.empty() ? I_SLICE : cPicNumList1.empty() ? P_SLICE : B_SLICE );             //根据LIST0和LIST1的情况决定Slice类型!!!!
  rcSliceHeader.setSliceType( eSliceType );

  //===== set re-ordering commands =====            参考队列重排序
  if( m_bUseLongTermPics )                //使用LongTerm参考帧   还没有看
  {
...
LayerEncoder::xSetSliceHeaderBase( SliceHeader& rcSliceHeader, UInt uiFrameIdInGOP, PicType ePicType )
{
  ...
  UInt        uiTemporalId    = m_auiFrameIdToTemporalId[ uiFrameIdInGOP ];                //得到时域的层序
  Bool        bFieldPic       = ( ePicType != FRAME );
  Bool        b2ndField       = ( ePicType == ( m_bBotFieldFirst ? TOP_FIELD : BOT_FIELD ) );            //指示是不是第二场
                                             //                      时域底层一定为参考帧                            该时域层在非丢弃的层内且不是最上面的一层          是否作参考
  Bool        bRefPic         = ( bFieldPic || uiTemporalId == 0 || m_uiDecompositionStages - uiTemporalId > m_uiNotCodedStages );
  NalRefIdc   eNalRefIdc      = ( !bRefPic ? NAL_REF_IDC_PRIORITY_LOWEST : uiTemporalId == 0 ? NAL_REF_IDC_PRIORITY_HIGHEST : uiTemporalId == 1 ? NAL_REF_IDC_PRIORITY_HIGH : NAL_REF_IDC_PRIORITY_LOW );  //安排NalRefIdc  优先级顺序
  Bool        bKeyPictures    = ( m_uiEncodeKeyPictures == 2 || ( m_uiEncodeKeyPictures == 1 && m_bMGS ) );
  Bool        bUseRefBasePic  = ( bKeyPictures && uiTemporalId == 0 );     //use_ref_base_flag   1 必须用基本层的做参考  0 用解码得到的图像做参考（BL+EL 或者BL【EL丢失】）
  Bool        bIdrFlag        = ( uiFrameIdInGOP == 0 && !b2ndField );               //GOP第一帧就是IDR -----  IDR条件1
  if( ! bIdrFlag && !b2ndField && m_iIDRPeriod != 0 )    //检查该帧是不是可以IDR
  {
    UInt      uiCurrFrameIdx  = ( ( m_uiGOPNumber << m_uiDecompositionStages ) + uiFrameIdInGOP ) << m_uiTemporalResolution;
    bIdrFlag                  = ( ( uiCurrFrameIdx % m_iIDRPeriod  ) == 0 );                      //GOP中到了IDR周期也是IDR -------  IDR条件2
    ROT( bIdrFlag && uiTemporalId > 0 );
  }
  if( bIdrFlag )            //IDR
  {
    m_uiFrameNum              = 0;                                     //IDR帧开始FrameNum重新从0开始计算
    m_uiNextTL0LongTermIdx    = 0;
    for( UInt uiFId = 0; uiFId <= ( 1U << m_uiDecompositionStages ); uiFId++ )             //清空
    {
      m_auiFrameIdToFrameNumOrLTIdx[ uiFId ] = MSYS_UINT_MAX;
    }
    for( UInt uiTId = 0; uiTId <= MAX_DSTAGES; uiTId++ )          //清空每个时间层上的参考队列
    {
      m_acActiveRefListFrameNum[ uiTId ].clear();
      m_abMaxLongTermIdxSend   [ uiTId ] = false;
    }
    m_cActiveRefBasePicListFrameNum.clear();              //清空该DependencyLayer的 reference base pic队列
  }                                                    //        基本层（ DependencyId = 0 ）时 IDR发nal_unit_type = 5   不是IDR发nal_unit_type=1      不是基本层时发nal_unit_type=20的
  ...
  Int         iLongTermIdx    = ( m_bUseLongTermPics && bRefPic ? ( uiTemporalId ? (Int)uiTemporalId + 1 : (Int)m_uiNextTL0LongTermIdx ) : -1 );   //LongTerm参考帧  暂时没看
  ...
  rcSliceHeader.setConstrainedIntraResamplingFlag ( m_bCIUFlag );             //此处是contrained_intra_upsampling_flag   每个层单独设置  还有PPS里的constrained_intra_pred_flag
  ...
  rcSliceHeader.setStoreRefBasePicFlag            ( bUseRefBasePic );                 //prefix_nal_unit_svc( )
  ...
  rcSliceHeader.setNoOutputOfPriorPicsFlag        ( false );                            //参考帧标记过程  dec_ref_pic_marking( )
  rcSliceHeader.setLongTermReferenceFlag          ( m_bUseLongTermPics && bIdrFlag );   //参考帧标记过程  dec_ref_pic_marking( )
...
{
  ROF( m_auiFrameIdToFrameNumOrLTIdx[ uiFrameIdInGOP ] == MSYS_UINT_MAX ); // frame_num must not be set
  UInt        uiTemporalId    = m_auiFrameIdToTemporalId[ uiFrameIdInGOP ];            //得到时间层号
  Bool        bFieldPic       = ( ePicType != FRAME );
  Bool        b2ndField       = ( ePicType == ( m_bBotFieldFirst ? TOP_FIELD : BOT_FIELD ) );
  Bool        bRefPic         = ( bFieldPic || uiTemporalId == 0 || m_uiDecompositionStages - uiTemporalId > m_uiNotCodedStages );   //判断做不做参考 =====>不是时间层最高的那一层即可
  Bool        bRefFrmOr2ndFld = ( bRefPic && ( !bFieldPic || b2ndField ) );
  Bool        bKeyPictures    = ( m_uiEncodeKeyPictures == 2 || ( m_uiEncodeKeyPictures == 1 && m_bMGS ) );        //2：CGS    1： MGS
  Bool        bUseRefBasePic  = ( bKeyPictures && uiTemporalId == 0 );        //当不当作reference base picture作参考
  ...
  UInt        uiCurrFrameNum  = rcSliceHeader.getFrameNum();           //FrameNum已经计算到SliceHeader里面去了
  UInt        uiCurrPicNum    = ( bFieldPic ? 2 * uiCurrFrameNum + 1 : uiCurrFrameNum );
  ...
  rcDecRefPicMarking    .clear( false );              //SliceHeader里
  rcDecRefBasePicMarking.clear( true );           //SliceHeader_SVC_Extension里

  //===== MMCO for short-term configurations =====
  if( ! m_bUseLongTermPics && uiFrameIdInGOP == m_uiFrameIdWithSmallestTId && bRefFrmOr2ndFld )   //MMCO
  {
  ...
  if( bRefFrmOr2ndFld )
  {
    m_uiFrameNum  = ( m_uiFrameNum + 1 ) % uiMaxFrameNum;           //下一帧的FrameNum   最后一个时域层的不会进来
    if( m_bUseLongTermPics )
    {
    ...
    else
    {
      ...
      m_acActiveRefListFrameNum [ uiTemporalId ].push_back( uiCurrFrameNum );     //每个DependencyLayer里每个时间层里面有效的作为参考的FrameNum
      if( bUseRefBasePic )
      {
        m_cActiveRefBasePicListFrameNum.push_back( uiCurrFrameNum );                  //可用来作为参考的reference base picture，因此存入其FrameNum、
      }
    }
  }
  if( bIdrFlag )
  {
    m_uiIdrPicId = ( m_uiIdrPicId + 1 ) % 3;                //得到下一个IdrPicId
  }
  return Err::m_nOK;
}
...
{
  SliceHeader* pcSliceHeader = m_pacControlData[ uiFrameIdInGOP ].getSliceHeader( ePicType );   //得到ControlData::SliceHeader*  m_pcSliceHeader;  
  ROF( pcSliceHeader );

  //===== set basic parameters, MMCO commands, and reordering commands =====
  RNOK( xSetSliceHeaderBase         ( *pcSliceHeader, uiFrameIdInGOP, ePicType ) );                     //set base information of SliceHeader
  RNOK( xSetSliceTypeAndRefLists    ( *pcSliceHeader, uiFrameIdInGOP, ePicType ) );                  //Reference List re-ordering commands
  RNOK( xSetMMCOAndUpdateParameters ( *pcSliceHeader, uiFrameIdInGOP, ePicType ) );      //MMCO settings     暂时没仔细看

  //===== de-blocking filter parameters =====
  if( pcSliceHeader->getPPS().getDeblockingFilterParametersPresentFlag() )     //设置滤波参数
  {
...
ErrVal
LayerEncoder::xInitSliceHeadersAndRefLists()    //对某个Dependency Layer
{
  //===== set number of frames to encode =====
  m_uiNumFramesLeftInGOP = ( m_uiGOPSize >> m_uiNotCodedStages ) + ( m_bFirstGOPCoded ? 0 : 1 );           //获得GOP未编码的帧数
  //===== reset parameters =====
  {
    for( UInt uiFrameId = 0; uiFrameId <= ( 1U << MAX_DSTAGES ); uiFrameId++ )      //重置该GOP里所有帧的参数
    {
      m_auiFrameIdToFrameNumOrLTIdx [ uiFrameId ] = MSYS_UINT_MAX;               //重置由FrameID得到FrameNum或LT层号的数组
      m_auiPicCodingType            [ uiFrameId ] = MSYS_UINT_MAX;
      m_acRefPicListFrameId_L0      [ uiFrameId ].clear();           //是个UInt的List  
  ...
  //===== determine frame id with lowest temporal level (first occurence) =====
  {
    ...
    m_uiFrameIdWithSmallestTId  = 0;              //找到最低时域层的帧号
    for( UInt uiFrameId = 1; uiFrameId <= m_uiGOPSize; uiFrameId++ )
    {
      if( m_auiFrameIdToTemporalId[ uiFrameId ] < uiSmallestTId )              //由FrameID得到GOP时域分层层号的数组
      {
  ...
  {
    UInt uiFrameIdInGOP  = m_auiCodingIndexToFrameId [ uiCodingIndex  ];   //按编码顺序 查找 帧播放顺序
    if(  uiFrameIdInGOP <= m_uiGOPSize )
    {
      UInt uiTemporalId  = m_auiFrameIdToTemporalId  [ uiFrameIdInGOP ];     //按帧播放顺序 查找 帧的时域分层号(从下到上递增)
      //----- reference lists -----   初始化参考队列 LayerEncoder类中
      RNOK( xInitRefFrameLists( uiFrameIdInGOP, uiTemporalId ) );   //传入参数：    帧播放顺序     帧的时域分层号
      //----- slice headers -----
      xPAffDecision( uiFrameIdInGOP );     //设置m_pbFieldPicFlag[ uiFrame ]
      if( m_pbFieldPicFlag[ uiFrameIdInGOP ] )                                      //有场才使用下面的  直接跳过
      {
...
ErrVal
LayerEncoder::xInitGOP( PicBufferList&  rcPicBufferInputList )                //这个函数是针对每个DependencyLayer的
{
  //========== INITIALIZE DECOMPOSITION STRUCTURES ==========
  m_uiGOPSize                         = (UInt)rcPicBufferInputList.size();
  PicBufferList::iterator cInputIter  = rcPicBufferInputList.begin();                //输入图像队列的迭代器起点
  UInt                    uiFrame     = 0;
  ...
  {
    m_uiGOPSize--;             //第一个GOP还没编码  因此现在是第一个GOP（多一个IDR，因此GOPSIZE = GOPSIZE + 1）
  }
  for( ; uiFrame <= m_uiGOPSize; uiFrame++, cInputIter++ )
  {
    if( m_pcResizeParameters->m_iExtendedSpatialScalability == ESS_PICT )      //与ESS_PICT相关的ESS参数设置  暂时没看
    {
    ...
    m_papcFrame					[ uiFrame ]->load             ( pcPicBuffer );   //存入Frame的成员YuvPicBuffer  m_cFullPelYuvBuffer中
    m_papcFrame					[ uiFrame ]->clearPoc         ();                   //清除之前的POC
    m_papcFrame					[ uiFrame ]->setPoc           ( iPoc );           //设置计算好的POC
    m_papcFrame                    [ uiFrame ]->setPicParameters ( *m_pcResizeParameters );   //Frame结构中的ResizeParameters*  m_pcResizeParameters;   这里的参数是ESS相关的SPS SVC扩展中的Resize信息
    if( m_papcELFrame )               //EL没有load
    {
  ...
  {
    RNOK( m_pcAnchorFrameOriginal ->copyAll           ( m_papcFrame[ m_uiGOPSize ] ) );     //该GOP最后一帧是下一个GOP需要用到的
  ...
  for( uiFrame = 0; uiFrame <= m_uiGOPSize; uiFrame++ )                      //LayerEncoder::ControlData*   m_pacControlData
  {
    m_pacControlData[ uiFrame ].clear();
    m_pacControlData[ uiFrame ].setScalingFactor( 1.0 );     //此句可以不要
    MbDataCtrl* pcMbDataCtrl = m_pacControlData[ uiFrame ].getMbDataCtrl();              //宏块级的控制类
    RNOK( pcMbDataCtrl->reset () );
    RNOK( pcMbDataCtrl->clear () );
  }

  RNOK( xInitSliceHeadersAndRefLists() );               //设置SliceHeader和参考队列(marking   reordering  MMCO)

  m_uiNewlyCodedBits += m_uiParameterSetBits;
  if( m_pcH264AVCEncoder->getCodingParameter()->getCGSSNRRefinement() )           //有MGS质量分层
  {
    m_pcH264AVCEncoder->m_aaadSeqBits[getLayerCGSSNR()][0][0] += m_uiParameterSetBits;   //m_aaadSeqBits[MAX_LAYERS][MAX_TEMP_LEVELS][MAX_QUALITY_LEVELS];
  }
  else
  {
  ...
  m_uiLastCodedFrameIdInGOP = MSYS_UINT_MAX;                //reset上一个编码的帧
  m_uiLastCodedTemporalId   = MSYS_UINT_MAX;                    //reset上一个编码的时间层
...
{
  UInt  uiLowerLayerBits             = m_pcH264AVCEncoder->getNewBits( m_uiBaseLayerId );           //借用了m_uiNewlyCodedBits
                                  ...
                                  UInt		uiTemporalId )
{
  ...
  if( m_uiLastCodedFrameIdInGOP != MSYS_UINT_MAX && m_uiLastCodedTemporalId == uiTemporalId )   //上一个编码的帧是同一个时间层的
  {
    bExists = true;
  }
                                    ...
                                    UInt		      uiTemporalId )
{
  ...
  if( m_uiLastCodedFrameIdInGOP != MSYS_UINT_MAX && m_uiLastCodedTemporalId == uiTemporalId )
  {
    ...
    pcFrame       = m_pcSubband;              //mark 查找BaseLayer可用性
    pcResidual    = m_pcResidualILPred;   //mark 查找BaseLayer可用性
    pcMbDataCtrl  = m_pacControlData[m_uiLastCodedFrameIdInGOP].getMbDataCtrl();    //mark 查找BaseLayer可用性
                                 ...
                                 PicType ePicType )
{
  ...
  if( ! m_bInterLayerPrediction )             //**************************设置inter-layer预测的信息 **********************
  {
  ...
  RNOK( m_pcH264AVCEncoder->getBaseLayerStatus( uiBaseLayerId, m_uiDependencyId, ePicType, uiTemporalId ) );  //检查BaseLayerId
  m_pcResizeParameters->updatePicParameters( m_papcFrame[ uiFrameIdInGOP ]->getPicParameters( ePicType ) );    //BaseLayer的ReSize信息
  ...
  if( bSendSkipSlices )              //SliceSkip  暂时没看  就是说如果所有的AU的reference layer representation都有，当前layer representation的slice_skip_flag = 1
  {
    bAdaptiveBaseMode = false;
    bAdaptiveMotion   = false;
    bAdaptiveResidual = false;
    bDefaultBaseMode  = true;
    bDefaultMotion    = true;				// 没有必要
  ...
  {
    PicType eTestPicType = ( m_pbFieldPicFlag[ m_uiGOPSize ] ? ( m_bBotFieldFirst ? BOT_FIELD : TOP_FIELD ) : FRAME );    //GOP最后一帧的帧格式
    if( m_pacControlData[ m_uiGOPSize ].getSliceHeader( eTestPicType )->getIdrFlag() )            //GOP最后一帧是否是IDR
    {
      UInt uiCodNumIDR  = 0;
      UInt uiCodNumCurr = 0;
      while( m_auiCodingIndexToFrameId[ uiCodNumIDR  ] != m_uiGOPSize )          //计数：按编码顺序，编码到GOP播放顺序上的最后一帧前编码帧个数
      {
        uiCodNumIDR++;
        ROF( uiCodNumIDR <= 64 );
      }
      while( m_auiCodingIndexToFrameId[ uiCodNumCurr ] != uiFrameIdInGOP )     //计数：按编码顺序，当前帧前面已编码帧个个数
      {
        uiCodNumCurr++;
        ROF( uiCodNumCurr <= 64 );
      }
      bAdaptiveBaseMode = ( uiCodNumIDR <= uiCodNumCurr );    //GOP最后一帧是IDR，除了GOP第一帧，都用AdaptiveBaseMode
      ...
      ( pcSliceHeader->getSPS().getConvertedLevelIdc() >= 30 || m_bBiPred8x8Disable ) )
  {
    ...
    UInt  uiBaseLevel           = 0;
    Bool  bBaseBiPred8Disable   = false;                 //temp    用于函数
    Bool  bBaseMCBlks8x8Disable = false;              //temp   用于函数
...
LayerEncoder::xUpdateELPics()
{
  ...
  for( UInt uiIndex = 0; uiIndex <= m_uiGOPSize; uiIndex++ )
  {
    if( m_papcELFrame[uiIndex]->isUnvalid() )
    {
      ...
      UInt          uiFIdInTId    = uiIndex >> uiShift;       //得到该帧在时间层上的ID
      const Frame*  pcELPic       = m_pcH264AVCEncoder->getELRefPic( m_uiDependencyId, uiTemporalId, uiFIdInTId );    //得到最高MGS增强层的参考图像
      if( pcELPic )
      {
        RNOK( m_papcELFrame[uiIndex]->copy( const_cast<Frame*>( pcELPic ), FRAME ) );
        RNOK( xFillAndUpsampleFrame( m_papcELFrame[uiIndex], FRAME, m_bFrameMbsOnlyFlag ) );       //上采样   暂时没看
...
LayerEncoder::getRefPic( UInt uiTemporalId, UInt uiFrameIdInTId )
{
  ...
  Frame* pcRefPic       = 0;
  if( m_abCoded[ uiFrameIdInGOP ] && m_pacControlData[ uiFrameIdInGOP ].getSliceHeader()->getNalRefIdc() )   //EL的帧得编码完成才能得到
  {
...
{
  for( UInt uiLevel = 0; uiLevel < m_uiDecompositionStages; uiLevel++ )           //对每个时间层  不包括最顶层
  {
    RNOK( xSetScalingFactors( uiLevel ) );             //设置 ControlData  m_pacControlData[ iFrame << uiBaseLevel ]的dScalingFactor
  }
  return Err::m_nOK;
}
...
LayerEncoder::xSetScalingFactors( UInt uiBaseLevel )
{
  ...
  Int     iLowPassSize    = ( m_uiGOPSize >> uiBaseLevel );      //GOP     GOP/2     GOP/4
  Int     iFrame;
...
LayerEncoder::xGetAndSetPredictionLists( ControlData& rcControlData,
                                         ...
                                         Bool         bHalfPel )
{
  RefListStruct&  rcRefListStruct = rcControlData.getRefListStruct();   //实际是   rcControlData.m_cRefListStruct;
  SliceHeader*    pcSliceHeader   = rcControlData.getSliceHeader( ePicType );
  ...
  RefListUsage    eRLU            = ( bKeyPic ? RLU_REF_BASE_PIC : RLU_RECONSTRUCTION );     //分别用m_apcBaseFrame[]    m_papcFrame[]

  //===== get prediction lists and set RefListStruct =====
  RNOK( xGetPredictionLists( rcRefList0, rcRefList1, uiFrameIdCol, uiFrameIdInGOP, ePicType, eRLU, bHalfPel ) );     //设置List0和List1的参考帧
  rcRefListStruct.uiFrameIdCol = uiFrameIdCol;             //暂时不知道  仅知道是List1第一个参考帧号
  if( bKeyPic || m_uiMGSKeyPictureControl == 0 )      //有KeyPic（时间层=0）或者MGSControl=0时，ME和MC用相同队列（当前MGS layer）
  {
  ...
  else            //用最高层队列
  {
    ...
    if( m_uiMGSKeyPictureControl == 1 )                     //MGSControl = 1时，  RC和MC使用相同的LIst（当前MGS layer）   而  ME使用的highest EL
    {
      rcRefListStruct.acRefFrameListMC[0].copy( rcRefList0 );
      rcRefListStruct.acRefFrameListMC[1].copy( rcRefList1 );
      rcRefListStruct.bMCandRClistsDiffer = false;
    }
    else            //MGSControl = 2时，  ME和MC使用相同的LIst（highest EL）
    {
  ...
  if( pcSliceHeader->isMbaffFrame() )               //跳过场部分 ********************************
  {
  ...
  if( pcSliceHeader->isH264AVCCompatible() && pcSliceHeader->getSliceType() == B_SLICE )
  {
    ...
    pcMbDataCtrl0L1 = m_pacControlData[ rcRefListStruct.uiFrameIdCol ].getMbDataCtrl();    //找到list1队列第一帧的ControlData的m_pcMbDataCtrl;
    ROF( pcMbDataCtrl0L1 );
  }
  rcControlData.setMbDataCtrl0L1( pcMbDataCtrl0L1 );     //设置m_pcMbDataCtrl0L1
                                   ...
                                   Bool           bHalfPel )
{
  ...
  UIntList&     rcFrameIdList0    = m_acRefPicListFrameId_L0[ uiFrameIdInGOP ];              //前向参考帧  按FrameId存储
  UIntList&     rcFrameIdList1    = m_acRefPicListFrameId_L1[ uiFrameIdInGOP ];              //后向参考帧
  ...
  {
    UInt   uiFrmId = *iter0;             //获得frameId
    Frame* pcFrame = xGetRefFrame( uiFrmId, eRefListUsage );              //获得对应index的参考帧
    if( !  pcFrame->isExtended() )
    {
      if( bHalfPel ) { RNOK( xFillAndUpsampleFrame( pcFrame, FRAME, bFrameMbsOnlyFlag ) ); }      //进行1/2像素的插值和margin填充
      else           { RNOK( xFillAndExtendFrame  ( pcFrame, FRAME, bFrameMbsOnlyFlag ) ); }
    }
    if( bFieldPic )      //场部分  不看************************
    {
    ...
    {
      RNOK( rcRefList0.add( pcFrame->getPic( FRAME ) ) );                    //把该ref帧加入list  (之前的只是）
    }
    pcFrame->setLongTerm( m_bUseLongTermPics );
  }
  //===== L1 list =====
                                  ...
                                  PicType       ePicType )
{
  ...
  RefFrameList* pcRefFrameList0     = &rcControlData.getRefListStruct().acRefFrameListRC[0];      //获得当前RC list
  ...
  if( rcControlData.getBaseLayerId() != MSYS_UINT_MAX )
  {
    ...
    if( bBaseDataAvailable )
    {
      ...
      RNOK( m_pcH264AVCEncoder->getBaseLayerData( *pcSliceHeader,                //获得BaseLayer的Frame和残差Frame
                                                  pcBaseFrame,
                                                  pcBaseResidual,
                                                  pcBaseDataCtrl,
  ...
  if( pcBaseDataCtrl )
  {
    ...
    RNOK( m_pcBaseLayerCtrl->initSlice( *pcSliceHeader, PRE_PROCESS, false, NULL ) );       //初始化BaseLayer的参考帧的slice   暂时没看
                                ...
                                PicType ePicType )
{
  ...
  //===== set base layer data =====
  RNOK( xSetBaseLayerData( uiFrameIdInGOP, ePicType ) );                  //设置BaseLayer的预测模式 检查LevelIdc等参数
  ...
  pcSliceHeader->setSliceHeaderQp ( iQPRes  );              //对残差使用的Qp  传统AVC的QP
  ...
  m_pcResizeParameters->updatePicParameters( m_papcFrame[ uiFrameIdInGOP ]->getPicParameters( ePicType ) );           //更新Resize信息
  RNOK( xInitBaseLayerData( rcControlData, ePicType ) );                       //********暂时没看  关于BaseLayer的 motion和residual的配置
  /* el & has correspongding bl frame & bl does wp then use bl wts    增强层 & BaseLayer采用WP*/
...
LayerEncoder::xSetCodingNumbers( UInt uiCodingIndex )
{
  ...
  if(  uiCodingIndex == 0 ) // first frame in sequence: reset all
  {
    for( UInt uiTId = 0; uiTId <= MAX_DSTAGES; uiTId++ )
    {
      ...
      RNOK( xGetInitialOutputDelay( m_auiInitialDelay[ uiTId ], uiTId ) );     //设置每个时间层的编码延时
    }
    return Err::m_nOK;
  }
  UInt  uiFrameIdInGop  = m_auiCodingIndexToFrameId [ uiCodingIndex   ];
...
ErrVal                   // ××××× 对每一个Layer GOP中的 每一个AU ×××××
LayerEncoder::xEncodePicture( Bool&           rbPictureCoded,              //是否已经编码
                              UInt            uiTemporalId,                                               //时间层ID
                              UInt            uiFrameIdInGOP,                                         //GOP中播放顺序
                              AccessUnitData& rcAccessUnitData,
                              PicBufferList&  rcPicBufferInputList )                        // H264AVCEncoder::m_acOrgPicBufferList  [uiLayer],    输入图像的list
{
  rbPictureCoded = false;
  ROTRS( uiFrameIdInGOP > m_uiGOPSize, Err::m_nOK );

  PicOutputDataList cPicOutputDataList;
  Bool    bFieldCoded     = m_pbFieldPicFlag[ uiFrameIdInGOP ] ;              //指示GOP中每帧是不是按Field编码
  ...
  {
    ::memset( m_abCoded, 0x00, sizeof( m_abCoded ) );                  // 如果是IDR    m_abCoded[(1<<MAX_DSTAGES)+1]都设置成0
  }
  m_abCoded[uiFrameIdInGOP] = true;                  //当前置true

  //===== check whether picture was already coded in last GOP =====
  if( uiFrameIdInGOP == 0 && m_uiGOPNumber )           //该帧是GOP第0个且已经有GOP被编码了
  {
    ...
    // JVT-R057 LA-RDO}
    return Err::m_nOK;                 //  此时rbPictureCoded = false;
  }
  else
  {
    rbPictureCoded = true;
  ...
  //JVT-W051 {                   SEI message
  if( uiFrameIdInGOP == 0 )
  {
    const SequenceParameterSet& rcSPS = m_pacControlData[ uiFrameIdInGOP ].getSliceHeader( eFirstPicType )->getSPS();   //从片头读SPS
  ...
  Frame*        pcFrame             = m_papcFrame     [ uiFrameIdInGOP ];              //原始帧数据
  Frame*        pcResidualLF        = m_pcResidualLF;                       //残差数据编码   
  Frame*        pcResidualILPred    = m_pcResidualILPred;
  Frame*        pcOrgFrame          = m_apcFrameTemp  [ 2 ];                              //临时帧数据
  Frame*        pcPredSignal        = m_apcFrameTemp  [ 3 ];
  ...
  for( UInt uiPicType = 0; uiPicType < uiNumPics; uiPicType++ )
  {
    ...
    ExtBinDataAccessorList& rcSEIOutputList   = rcAccessUnitData.getSEINalUnitList();                   //SEI的NalUnit list
    ExtBinDataAccessorList& rcOutputList      = rcAccessUnitData.getNalUnitList();                           //真正的NalUnit list
    ExtBinDataAccessorList& rcRedOutputList   = rcAccessUnitData.getRedNalUnitList();                //Redundent Picture的NalUnit list
    PicType                 ePicType          = ( uiPicType ? eLastPicType : eFirstPicType );
    SliceHeader*            pcSliceHeader     = rcControlData.getSliceHeader( ePicType );
    ROF( pcSliceHeader );
	//JVT-W047  Layer with the highest dependency_id may be coded with a lower temporal resolution than its base layer
    ...
    //===== non-required layer representation SEI =====          暂时不关注这个**********START********************************
    ...
    // JVT-W049 }                                                 暂时不关注这个***************END*************************


    //===== update statistics =====
    if( m_pcH264AVCEncoder->getCodingParameter()->getCGSSNRRefinement() )             //MGS
    {
    ...
    if( pcSliceHeader->getPPS().getRedundantPicCntPresentFlag() )                  //暂时不看 redundant picture
    {
    ...
    {
      ROT( pcSliceHeader->getRedundantPicCnt() );                   //该slice之前不能有redundant picture
      // JVT-R057 LA-RDO{                              暂时没看 LA-RDO **********************************START***************************
      if( m_bLARDOEnable )
      {
      ...
      // JVT-R057 LA-RDO}                             暂时没看 LA-RDO **********************************END***************************
      ...
      RNOK( xEncodeLayerRepresentation( rcOutputList, rcControlData,                //rcAccessUnitData.getNalUnitList() / m_pacControlData[ uiFrameIdInGOP ];        
                                        pcOrgFrame, pcFrame, pcResidualLF, pcResidualILPred, pcPredSignal,  //m_apcFrameTemp[ 2 ] /  m_papcFrame[ uiFrameIdInGOP ] / m_pcResidualLF / m_pcResidualILPred / m_apcFrameTemp[ 3 ];
                                        uiBits, cPicOutputDataList, uiFrameIdInGOP, ePicType ) );     //计算编码比特数  / 输出图像list / GOP内第几帧 / FRAME
    ...
    //===== update parameters =====
    m_uiNewlyCodedBits += uiBits;              //保存每一个layer编码的比特数
...
{
  ROT   ( m_bGOPInitialized );          //一开始肯定没有初始化
  ROTRS ( rcPicBufferInputList.empty(), Err::m_nOK );            //输入图像队列为空则报错
  RNOK  ( xInitGOP              ( rcPicBufferInputList ) );              //初始化GOP  SliceHeader  RefList   POC等信息
  RNOK  ( xSetScalingFactors    () );                          //question  ?
  RNOK  ( xClearELPics          () );                          //使m_papcELFrame[]  unvalid
  ::memset( m_abCoded, 0x00, sizeof( m_abCoded ) );
  if( m_bFirstGOPCoded )           //要第一个GOP编码了才行
  {
    //==== copy picture zero =====
    PicOutputDataList cPicOutputDataList;
  ...
  m_bGOPInitialized = true;             //GOP初始完毕
  return Err::m_nOK;
}
...
// JVT-V068 HRD }
  ...
  // JVT-W043 }
  RNOK( xInitBitCounts() );       //更新m_uiNewlyCodedBits

  //===== update higher layer pictures =====
  RNOK( xUpdateELPics() );             //得到最高增强层（MGS，同分辨率）的参考图像

	//JVT-X046 {           一个frame划分成多个slice
  m_pcSliceEncoder->m_uiSliceMode = m_uiSliceMode;
  m_pcSliceEncoder->m_uiSliceArgument = m_uiSliceArgument;
  //JVT-X046 }

  UInt   uiCodingIndex   = ( m_bFirstGOPCoded ? uiAUIndex + 1 : uiAUIndex );      //GOP中编码顺序   第一个GOP是GOPSIZE+1个
  UInt   uiFrameIdInGOP  = m_auiCodingIndexToFrameId[ uiCodingIndex  ];
  ROTRS( uiFrameIdInGOP  > m_uiGOPSize, Err::m_nOK );
  RNOK( xSetCodingNumbers( uiCodingIndex ) );                        //编码一帧后计数   具体意义 ???? question
  UInt   uiTemporalId    = m_auiFrameIdToTemporalId [ uiFrameIdInGOP ];
  UInt   uiSliceMode     = m_uiSliceMode;
  UInt   uiSliceArgument = m_uiSliceArgument;
  if(    uiSliceMode != 2 )            //设置SliceSize  应该跟JVT-Q066有关  暂时没看
  {
  ...
  Bool  bPicCoded = false;              // AU     时间层                GOP顺序               AU数据类                  输入图像list   
  RNOK( xEncodePicture( bPicCoded, uiTemporalId, uiFrameIdInGOP, rcAccessUnitData, rcPicBufferInputList ) );    //对每一帧 按每一层编码
  ROF ( bPicCoded );  
...
{
  UInt uiBit = 0;
  Bool m_bWriteSuffixUnit = true;      //多此一举
  if( m_bWriteSuffixUnit )
  {
    RNOK( xInitExtBinDataAccessor        (  m_cExtBinDataAccessor ) );            //与LayerEncoder 中的 BinData  m_cBinData关联起来
    RNOK( m_pcNalUnitEncoder->initNalUnit( &m_cExtBinDataAccessor ) );    //m_cExtBinDataAccessor 是操纵LayerEncoder的比特流用的  现在与NalUnit关联了

    NalUnitType eNalUnitType = rcSH.getNalUnitType();
    UInt eLayerId = rcSH.getDependencyId();

    rcSH.setDependencyId( 0 );               //只有BL才写这个
    ...
    RNOK( m_pcNalUnitEncoder->writePrefix( rcSH ) );        //写到了  m_pcNalUnitEncoder 中的 m_pcHeaderSymbolWriteIf变量中

    RNOK( m_pcNalUnitEncoder->closeNalUnit( uiBit ) );     //与NalUnitEncoder 脱离关系  数据仍在  m_cExtBinDataAccessor 中
    RNOK( xAppendNewExtBinDataAccessor( rcOutExtBinDataAccessorList, &m_cExtBinDataAccessor ) );      //把NalUnit的数据存入List中
    uiBit += 4*8;                    //这个是?
    ruiBit += uiBit;
