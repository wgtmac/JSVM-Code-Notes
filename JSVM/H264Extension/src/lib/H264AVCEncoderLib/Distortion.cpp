...
{
  m_cOrgData.loadBuffer( pcOrgYuvBuffer );      //从YuvPicBuffer 拷贝到 YuvMbBuffer
...
UInt XDistortion::get8x8Cr( XPel *pPel, Int iStride, DFunc eDFunc )
{
  XDistSearchStruct cDSS;
  getDistStruct( BLK_8x8, eDFunc, false, cDSS );
  cDSS.pYOrg    = m_cOrgData.getMbCrAddr();   // getDistStruct( BLK_8x8, eDFunc, false, cDSS );里已经设置为  m_cOrgData.getLumBlk();
  cDSS.pYSearch = pPel;     //重建的Cr亮度宏块起始
  cDSS.iYStride = iStride;
  return cDSS.Func( &cDSS );      //计算 Distortion
}
...
UInt XDistortion::getLum16x16( XPel *pPel, Int iStride, DFunc eDFunc )
{
  ...
  cDSS.pYSearch = pPel;
  cDSS.iYStride = iStride;
  return cDSS.Func( &cDSS );     //计算宏块的distortion
}
