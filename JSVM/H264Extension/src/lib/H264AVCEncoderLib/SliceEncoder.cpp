...
ErrVal
SliceEncoder::encodeSliceSVC( ControlData&  rcControlData,        // control data                        m_pacControlData[ uiFrameIdInGOP ]
                              Frame&        rcOrgFrame,           // original frame                                                  m_apcFrameTemp[ 2 ]
                              Frame&        rcFrame,              // reconstructed frame                                           m_papcFrame[ uiFrameIdInGOP ]
                              Frame*        pcResidualFrameLF,    // reconstructed residual for loop filter       m_pcResidualLF
                              Frame*        pcResidualFrameILPred,// reconstructed residual for inter-layer prediction             m_pcResidualILPred
                              Frame*        pcPredFrame,          // prediction signal                                                                          m_apcFrameTemp[ 3 ]
                              PicType       ePicType,             // picture type                                                                                      FRAME
                              ...
                              )
{
  ...
  Frame&  rcOrgPic            = *rcOrgFrame.getPic( ePicType );                  //原始图像
  Frame&  rcPic               = *rcFrame   .getPic( ePicType );                         //重建图像
  ...
  if( pcBaseMbDataCtrl )                  //有参考BaseLayer
  {
    RNOK( pcBaseMbDataCtrl->initSlice         ( rcSliceHeader, PRE_PROCESS,    false, NULL ) );
  }
  RNOK( m_pcControlMng    ->initSliceForCoding( rcSliceHeader ) );                         //重要函数 初始化slice级别的编码环境   P 和 B部分还没看
  ...
  for( m_pcMbCoder->bSliceCodedDone = false; uiMbAddress <= uiLastMbAddress; ) //--ICU/ETRI FMO Implementation
  {
    ...
    if( bRateControlEnable && !pcJSVMParams->m_uiLayerId )     // RateControl部分 暂时没看  *******************START**********************
    {
    ...
    RNOK( pcMbDataCtrl      ->initMb              ( pcMbDataAccess,     uiMbY, uiMbX ) );   //初始化的数据在 pcMbDataAccess 中
    if  ( pcBaseMbDataCtrl )
    {
      RNOK( pcBaseMbDataCtrl->initMb              ( pcMbDataAccessBase, uiMbY, uiMbX ) );    //把baselayer的信息初始化并存入pcMbDataAccessBase
  	}
    RNOK( m_pcControlMng    ->initMbForCoding     ( *pcMbDataAccess,    uiMbY, uiMbX, false, false  ) );      //重要函数 初始化MB级别的编码环境 
    pcMbDataAccess          ->setMbDataAccessBase ( pcMbDataAccessBase );   //pcMbDataAccessBase 与 pcMbDataAccess 绑定
    if  ( ! rcSliceHeader.getNoInterLayerPredFlag () )    //如果支持 inter-layer
    {
    ...
    {
      pcMbDataAccess->getMbData().setQp           ( rcSliceHeader.getSliceQp() );      //设置宏块level的QP = delatsliceQP + ppsQP
	...
	/*
	 MbDataAccess&   rcMbDataAccess,               // current macroblock data   
	...
	Frame*          pcResidualILPred,                         // reconstructed residual for inter-layer prediction    注意与上面的区别(猜是LP以后的残差)
	...
	*/
    RNOK( m_pcMbEncoder->encodeMacroblockSVC( *pcMbDataAccess, pcMbDataAccessBase,     // 该函数执行完后保存了最佳预测模式的数据
