...
H264AVCEncoder::getELRefPic( UInt uiLayerId, UInt uiTemporalId, UInt uiFrameIdInTId )
{
  ...
  for( UInt uiELId = uiLayerId + 1; uiELId < m_pcCodingParameter->getNumberOfLayers(); uiELId++ )
  {
    ...
    LayerParameters&  rcCL  = m_pcCodingParameter->getLayerParameters( uiLayerId  );         //当前Layer
    LayerParameters&  rcEL  = m_pcCodingParameter->getLayerParameters( uiELId     );            // EL的参数
    if( ! rcEL.getResizeParameters().getCroppingFlag() &&
        ...
        rcCL.getResizeParameters().m_iExtendedSpatialScalability == ESS_NONE )                  //不用Resize
    {
      pcELRefPic = m_apcLayerEncoder[uiELId]->getRefPic( uiTemporalId, uiFrameIdInTId );      //得到相同分辨率最高MGS增强层层且已编码的RefPic
    }
    else
    {
      break;
...
ErrVal
H264AVCEncoder::getBaseLayerStatus( UInt&   ruiBaseLayerId,           //确认BaseLayer是否存在（指的是用来做参考的Layer）
                                    UInt    uiLayerId,
                                    PicType ePicType,
                                    UInt		uiTemporalId )
{
...
H264AVCEncoder::getNewBits( UInt uiBaseLayerId )
{
  ...
  return m_apcLayerEncoder[uiBaseLayerId]->getNewBits();      //m_uiNewlyCodedBits
}
...
H264AVCEncoder::writeParameterSets( ExtBinDataAccessor* pcExtBinDataAccessor,
                                    ...
                                    Bool &rbMoreSets )
{
  if( m_bVeryFirstCall )            //第一次调用写参数集的函数
  {
    m_bVeryFirstCall = false;    //以后就不是第一次调用了

    if( ! m_bScalableSeiMessage )
    {
      RNOK( xInitParameterSets() );            //初始化参数集
    }
    else
    {
      RNOK( xWriteScalableSEI( pcExtBinDataAccessor ) );              //SEI优先于参数集
    }

    LayerParameters& rcLayer = m_pcCodingParameter->getLayerParameters ( 0 );
    if (0 < rcLayer.getNumROI())                    //ROI extraction   与 SEI 有关
      m_bWrteROISEI = true;
    else
      m_bWrteROISEI = false;
    m_loop_roi_sei=0;
  ...
  if( ! m_cUnWrittenSPS.empty() )            //有没写到NalUnit的SPS
  {
    RNOK( m_pcNalUnitEncoder->initNalUnit( pcExtBinDataAccessor ) );        //调用成员NalUnitEncoder*  m_pcNalUnitEncoder进行NAL的写入
    ...
    RNOK( m_pcNalUnitEncoder->write( rcSPS ) );              //把SPS写到NAL单元中去
    RNOK( m_pcNalUnitEncoder->closeNalUnit( uiBits ) );
    ...
    if( m_pcCodingParameter->getNumberOfLayers() )
    {
      ...
      RNOK( m_apcLayerEncoder[uiLayer]->addParameterSetBits( 8*(uiSize+4) ) );         //LayerEncoder*   m_apcLayerEncoder中统计SPS的比特数
    }
...
// JVT-V068 }
    ...
    m_cUnWrittenSPS.pop_front();     //弹出刚写的SPS
  }
  else
  {
  ...
  rbMoreSets = !(m_cUnWrittenSPS.empty() && m_cUnWrittenPPS.empty());   //判断还有没有没写的SPS和PPS
  return Err::m_nOK;
}
...
{
  UInt  uiHighestLayer  = m_pcCodingParameter->getNumberOfLayers() - 1;             //最高层的DependencyID
  //getDecompositionStages()返回时域分层(GOP)层数   如果第一个GOP没编码完成  则第一次需要GOP+1个帧（第一个IDR帧）
  ...
  {
    if( apcOriginalPicBuffer[ uiLayer ] )             //读取了该帧
    {
      ROF( apcReconstructPicBuffer[ uiLayer ] );
      m_acOrgPicBufferList  [ uiLayer ].push_back( apcOriginalPicBuffer   [ uiLayer ] );            //将原始图像和重建图像的数组存入list中
      m_acRecPicBufferList  [ uiLayer ].push_back( apcReconstructPicBuffer[ uiLayer ] );
    }
    else if( apcReconstructPicBuffer[ uiLayer ] )    //该帧被skip了 不读取 但仍然要恢复出来
    {
  ...
  if ( m_acOrgPicBufferList[uiHighestLayer].size() == uiTargetSize )       //读完需要的GOP大小
  {
                             ...
                             PicBufferList* apcPicBufferUnusedList )
{
  ...
  //===== init GOP =====
  for( uiLayer = 0; uiLayer <  m_pcCodingParameter->getNumberOfLayers(); uiLayer++ )
  {    //设置每个DependencyLayer的质量分层ID         h264::H264AVCEncoder的LayerEncoder*  m_apcLayerEncoder   [MAX_LAYERS];
    ...
    //h264::H264AVCEncoder的AccessUnitDataList m_cAccessUnitDataList;   有std::list<AccessUnitData>  m_cAccessUnitDataList;
  ...
  //JVT-W052                 SVC feedback based coding  ： allow store_base_rep_flag = 1 when  use_base_prediction_flag = 0   暂时没看
  ...
  {																																									//JVT-W052
    if(m_pcCodingParameter->getIntegrityCheckSEIEnable() && m_pcCodingParameter->getCGSSNRRefinement())//JVT-W052   暂时没看
    {
    ...
    for( uiLayer   = 0; uiLayer   <  m_pcCodingParameter->getNumberOfLayers(); uiLayer  ++ )    //对每一层
    {
      if( m_pcCodingParameter->getNonRequiredEnable() )            //non-required picture SEI message   JVT-Q066
      {
      ...
      m_bIsFirstGOP = !m_apcLayerEncoder[uiLayer]->firstGOPCoded();      //是否是第一个GOP
      RNOK( m_apcLayerEncoder[uiLayer]->process( uiAUIndex,
                                                ...
                                                m_pcParameterSetMng ) );                                       // ××××× 对每一个Layer GOP中的 每一个AU ×××××
      //JVT-W051 {
      if ( m_bIsFirstGOP )
      {
  ...
  {
    //----- set output list -----
    apcPicBufferOutputList[ uiLayer ] += m_acRecPicBufferList[ uiLayer ];                          //设置输出图像的 list
    //----- update unused list -----
    apcPicBufferUnusedList[ uiLayer ] += m_acOrgPicBufferList[ uiLayer ];                          // 设置 unused 的list
...
H264AVCEncoder::xInitParameterSets()
{
  ...
  //===== determine values for Poc calculation =====               question：不同层的POC计算方式  及其与POC表示位数关系  ???????????
  ...
  for( uiIndex = 0; uiIndex < m_pcCodingParameter->getNumberOfLayers(); uiIndex++ )
  {
    ...
    UInt              uiMbY               = rcLayerParameters.getFrameHeightInMbs();         //Y方向宏块个数
    UInt              uiMbX               = rcLayerParameters.getFrameWidthInMbs ();         //X方向宏块个数
    UInt              uiCropLeft          = 0;            //这里的crop只是将视频大小补成16的倍数
    UInt              uiCropTop           = 0;
    ...
    UInt              uiKeyPicMode        = m_pcCodingParameter->getEncodeKeyPictures ();      //0无 |  1 MGS (Q>0) refinement  |  2 temporal level 0
    Bool              bMGS                = ( m_pcCodingParameter->getCGSSNRRefinement() != 0 );     //代表每个SNR层分级方式---CGS/MGS
    Bool              bKeyPictures        = ( uiKeyPicMode == 2 || ( bMGS && uiKeyPicMode == 1 ) );    //是否使用关键帧
    UInt              uiPaffOffset        = ( rcLayerParameters.getPAff() ? 1 : 0 );
    UInt              uiRefBasePicOffset  = ( bKeyPictures ? 1 + uiPaffOffset : 0 );           //question  ?????????????????
    ...
    UInt              uiDPBSize           = uiNumRefPic;              // QUESTION
    ...
    if( rcLayerParameters.getUseLongTerm() )
    {
      ...
      uiNumRefPic = uiDecStages + 1 + uiPaffOffset + ( bKeyPictures ? 2 : 0 );    //question   加入长期参考后参考图像数量怎么变化 ?????????????????
      uiDPBSize   = uiNumRefPic;
    }
    else if( !rcLayerParameters.getMMCOEnable() )           //不使用MMCO
    {
    ...
    { //>>>>> subclause G.10.2.1 >>>>>
      ...
      UInt              uiLayerCount  = 0;
      UInt              uiBaseDId     = rcCurrLayer.getBaseLayerCGSSNR();             // for lowest MGS fragment        BL的DID
      UInt              uiBaseQId     = rcCurrLayer.getBaseQualityLevelCGSSNR();      // for lowest MGS fragment      BL的QID
      UInt              uiNumMGSFrag  = rcCurrLayer.getNumberOfQualityLevelsCGSSNR(); // number of MGS fragments in "layer"      当前空间层SNR分层数

	  //这里是对SNR分级的MGS层进行统计    SNR至少分三层 ?? 这里有疑问 因为下面同样使用这个值的时候+1了  目的：统计ref层总共的宏块数
	  ...
	  //这里是对每个dependency layer的SNR层进行统计
      for( UInt uiRefLayerId = uiIndex - 1; uiBaseDId != MSYS_UINT_MAX; uiRefLayerId-- )          // 从前面每一层中找到参考层
      {
        ...
        if( uiRefDId == uiBaseDId && uiRefQId <= uiBaseQId && uiRefQId + uiRefMGSFrag > uiBaseQId )    //找到了参考层
        {
          uiNumMGSFrag = uiBaseQId - uiRefQId + 2;     //故意+1了 因此--uiNumMGSFrag操作忽略了第一层(照这样理解  上面忽略了2层；如果这里没有忽略 那上面就只忽略了一层基本层  这种理解似乎更靠谱)
          while( (--uiNumMGSFrag) > 0 )
          {
            if( (++uiLayerCount) > 1 )
            {   //uiRefLayerMbs代表增强层总共的宏块数
              uiRefLayerMbs += rcRefLayer.getFrameHeightInMbs() * rcRefLayer.getFrameWidthInMbs();           //结果：      5    1  ==>     4   0           3   1  ==>  2   0      3   5   1 ==>  7  4  0     递归计算
            }
          }
    ...
    //----- get level idc -----
    UInt uiLevelIdc = SequenceParameterSet::getLevelIdc( uiMbY, uiMbX, uiOutFreq, uiMvRange, uiDPBSize, uiRefLayerMbs );         //选择适合的level_idc
    RNOK( rcLayerParameters.updateWithLevel( m_pcCodingParameter, uiLevelIdc ) );          //与基本层和Profile信息比对后得到新的level
    ...
    RNOK( m_pcParameterSetMng->store( pcPPS ) );      //因为号码会从0-31循环  因此把原来缓存中对应号码的SPS/PPS存入list中
    ...
    for(  UInt ui = 0; ui < uiNumQ; ui++ )             //对该dependency layer的每个SNR层，  可见每个SNR层都有独立的SPS 
    {
      m_pcParameterSetMng->setActiveSPS( pcSPS->getSeqParameterSetId(), uiDQId + ui );    //m_auiActiveSPSId[16×8] 以DQId索引  8个dependency layer   每个最多16层SNR  存储每层的激活SPS   共享相同的SPS/subsps的id
    }
    ...
    if( uiIndex == 0 )       //JVT-V035   SVC to AVC 的重写  其实他们不在SPS中
    ...
    // JVT-V068 HRD {                   Hypothetical Reference Decoder         暂时没看   与VUI有关
    pcSPS->setVUI( pcSPS );
    ...
    //SSPS {
		if( uiIndex ) // sub-SPS
		{
      pcSPS->setAVCHeaderRewriteFlag( false );              //svc to avc的重写
			pcSPS->setSVCVUIParametersPresentFlag( ( m_pcCodingParameter->getEnableVUI()!=0 ) );   //对比可见只有refinement layer才能有VUI
			pcSPS->setAdditionalExtension2Flag( false);     //仍不知是啥
	  ...
	  //SSPS }
    if( rcLayerParameters.getQualityLevelCGSSNR() ) // MGS enhancement           -----  CGS增强层的该值永远为0
    {
      ...
      RNOK( m_pcParameterSetMng->getActiveSPS   (  pcQ0SPS, uiCurrDQId - 1 ) );            //为什么是-1?
      RNOK( pcSPS->copySPSDataForMGSEnhancement ( *pcQ0SPS ) );     // MGS增强层的参数直接从前面MGS层的SPS里拷贝
    }


    //===== set picture parameter set parameters =====    SPS已经完成  现在是PPS
    ...
    // JVT-W043 {          仅给BaseLayer的RateControl
    ...
    {
      pcPPS->getPicScalingMatrix().init             ( rcLayerParameters.getScalMatrixBuffer() );        //获得人工指定的scalinglist
    }
...
//TMM_WP
  ...
  if( bOutput )
  {
    ...
    for( UInt uiLayer = 0; uiLayer < m_pcCodingParameter->getNumberOfLayers(); uiLayer++ )
    {
      ...
      UInt              uiDQ  = ( rcL.getLayerCGSSNR() << 4 ) + rcL.getQualityLevelCGSSNR();     //空间层id*16+质量层id
      UInt              uiNX  = rcL.getNumberOfQualityLevelsCGSSNR();                     //质量层数(代码里就是MGS的层数)
  ...
  /*
  从ParameterSetMng*  m_pcParameterSetMng中的SPS和PPS缓存m_cSPSBuf和m_cPPSBuf中
  读取并保存到 
