...
ErrVal ReadYuvFile::init( const std::string& rcFileName, UInt uiLumPicHeight, UInt uiLumPicWidth, UInt uiStartLine, UInt uiEndLine, FillMode eFillMode )
{
  ...
  if( Err::m_nOK != m_cFile.open( rcFileName, LargeFile::OM_READONLY ) )   //又用LargeFile文件类进行文件操作 调用底层的open等文件操作
  {
...
ErrVal ReadYuvFile::readFrame( UChar *pLum,              //这里调用了 operator (Pel*)
                               UChar *pCb,
                               UChar *pCr,
                               UInt uiBufHeight,
                               UInt uiBufWidth,
