...
ErrVal EncoderCodingParameter::init( Int     argc,
                                     ...
                                     std::string& rcBitstreamFile  )
{
  Char* pcCom;            //作为一个临时变量，存储各个命令参数字符串

  rcBitstreamFile = "";           //这个参数为函数形参

  ROTS( argc < 2 )              //至少要有两个参数   一个是程序名

  for( Int n = 1; n < argc; n++ )
  {
    pcCom = argv[n++];             //直接赋值字符串首地址，用的C风格字符串
    ...
    if( equals( pcCom, "-rqp", 4 ) )					// 某一个layer的base QP / keypicture的ME和MD的qp
    {
      ROTS( NULL == argv[n  ] );
      ROTS( NULL == argv[n+1] );
      UInt    uiLayer = atoi( argv[n  ] );
    ...
    if( equals( pcCom, "-mqp", 4 ) )			// qp for ME MD
    {
      ROTS( NULL == argv[n  ] );
      ROTS( NULL == argv[n+1] );
      ROTS( NULL == argv[n+2] );
    ...
    if( equals( pcCom, "-lqp", 4 ) )			// 所有的ME和MD的QP设置成一样
    {
      ROTS( NULL == argv[n  ] );
      ROTS( NULL == argv[n+1] );
      UInt    uiLayer = atoi( argv[n  ] );
    ...
    if( equals( pcCom, "-meqplp", 7 ) )				// keypicture的ME和MD的qp
    {
      ROTS( NULL == argv[n  ] );
      ROTS( NULL == argv[n+1] );
      UInt    uiLayer = atoi( argv[n  ] );
    ...
    if( equals( pcCom, "-blid", 5 ) )			// BaseLayerId
    {
      ROTS( NULL == argv[n  ] );
      ROTS( NULL == argv[n+1] );
      UInt    uiLayer = atoi( argv[n  ] );
    ...
    if( equals( pcCom, "-pf", 3) )                      //底层用strcmp，比较前三个字符
    {
      ROTS( NULL == argv[n] );
      std::string cFilename = argv[n];              //用C风格字符串给string对象赋值，获得配置参数文件路径
    ...
    if( equals( pcCom, "-ciu", 3 ) )				// slice边界 影响IntraBase的上采样和去块滤波
    {
	    ROTS( NULL == argv[n] );
...
ErrVal EncoderCodingParameter::xReadLine( FILE* hFile, std::string* pacTag )
  {
  ROF( pacTag );    //地址为空则出错   是string数组

    Int  n;
    UInt uiTagNum = 0;          //记录已经存储几个字段了
  Bool          bComment  = false;
  std::string*  pcTag     = &pacTag[0];

    for( n = 0; n < 4; n++ )              //为什么要4个？
    {
      pacTag[n] = "";
    }
  ...
  for( n = 0; ; n++ ) 
  {
      Char cChar = (Char) fgetc( hFile );            //从文件中读入一个字符
    ...
    if( ! bComment )
	 {  
		 if ( cChar == '\t' || cChar == ' ' ) // white space
		 {
			 ...
			 if( ! pcTag->empty() )       //当前存储用的字符串非空  表示已经存了相应内容
			 {
				 uiTagNum++;             //指示是这一行第几个内容  只能为0   1  2  3四个值   且等于4时，后面不能接空白字符了
				 pcTag = &pacTag[uiTagNum];           //换下一个字符串
			 }
		 }
		 else
		 {
...
{
  std::string acLayerConfigName[MAX_LAYERS];        //每层对应的配置文件路径
  std::string acTags[4];                         //用来读取一行配置文件所用的临时字符串
  UInt        uiLayerCnt   = 0;                  //分几层
  UInt        uiParLnCount = 0;               //配置参数个数
  ...
  // m_pEncoderLines[]是CodingParameter继承给EncoderCodingParameter的成员
  //                                                                                                  参数名        取地址后，该参数与m_pEncoderLines[]中对应参数保持联系  默认值
...
//JVT-W049 }
  ...
  //上面都是设置默认参数值，这里才是从配置文件中读取相应参数
  while (!feof(f))
  {
    RNOK( xReadLine( f, acTags ) );            //读取配置文件一行内容，存入acTags[]中，共4个
    if ( acTags[0].empty() )
    {
      continue;           //空行
    }
    ...
    {
      if( acTags[0] == m_pEncoderLines[ui]->getTag() )          //找到相同内容的配置参数(读取的和之前分配在m_pEncoderLines中的)
      {
        m_pEncoderLines[ui]->setVar( acTags[1] );              //更改新的参数值
        break;
      }
    }
    if( acTags[0] == "LayerCfg" )            //读取到每层的配置文件路径
    { 
  ...
  //回收用来临时存储配置文件参数的变量m_pEncoderLines[]，对应参数以及存入了CodingParameter类型成员里面了
  uiParLnCount = 0;
...
//JVT-T054}
  ...
  for( ui = 0; ui < m_uiNumberOfLayers; ui++ )
  {
	...
	//每个Dependency层id就是对应几个Layer的号
    getLayerParameters(ui).setDependencyId(ui);        //每层的参数在EncoderCodingParameter中的m_acLayerParameters[n]成员中  参数是废的？
    
	RNOK( xReadLayerFromFile( acLayerConfigName[ui], getLayerParameters(ui) ) );          //从每层的配置文件中读取参数  存入m_acLayerParameters[n]
    ...
    if( ui && uiBaseLayerId == MSYS_UINT_MAX )				// 不可能出现除基本层外的其他为MAX
    {
    ...
    if( m_uiCGSSNRRefinementFlag )   //      MGS --------------------------------------START--------------------------------------------------
    {
      if(ui == 0)        //MGS第一层，是BL
      {
        uiPrevLayer  = ui;
        dPrevTemp    = getLayerParameters(ui).getOutputFrameRate();           //这三个变量是给下一层使用的
      ...
      else
      {
        if( dPrevTemp    == getLayerParameters(ui).getOutputFrameRate     () &&
            uiPrevWidth  == getLayerParameters(ui).getFrameWidthInSamples () &&
            uiPrevHeight == getLayerParameters(ui).getFrameHeightInSamples() )                //前一层和这一层分辨率相同  属于MGS 仍是同一个DependencyLayer
        {
          // layer can be considered as a CGS refinement
          UInt uiLayerTemp = getLayerParameters(ui-1).getLayerCGSSNR();
          getLayerParameters(ui).setLayerCGSSNR(uiLayerTemp);                    //和前一层CGSSNR一样，表示属于同一个空间层的不同质量层
          UInt uiQualityLevelTemp = getLayerParameters(ui-1).getQualityLevelCGSSNR();
          getLayerParameters(ui).setQualityLevelCGSSNR( uiQualityLevelTemp + getLayerParameters(ui-1).getNumberOfQualityLevelsCGSSNR() );    //接着BaseLayer的QID+1往下存QID
        ...
        else
        {
          ...
          uiPrevWidth  = getLayerParameters(ui).getFrameWidthInSamples ();
          uiPrevHeight = getLayerParameters(ui).getFrameHeightInSamples();
          getLayerParameters(ui).setLayerCGSSNR(uiLastLayer);             //新的一个DependencyLayer
          getLayerParameters(ui).setQualityLevelCGSSNR(0);                  //质量层从0开始
        }
    ...
    else          //CGS SPATIAL *************************************************START*************************************************
    {
      getLayerParameters(ui).setLayerCGSSNR(ui);
      getLayerParameters(ui).setQualityLevelCGSSNR(0);           //CGS和Spatial 都是每层Quality_id = 0开始
      if( ui )
      {
        getLayerParameters(ui).setBaseLayerCGSSNR( getLayerParameters(uiBaseLayerId).getLayerCGSSNR() );
        getLayerParameters(ui).setBaseQualityLevelCGSSNR( getLayerParameters(uiBaseLayerId).getQualityLevelCGSSNR() + getLayerParameters(uiBaseLayerId).getNumberOfQualityLevelsCGSSNR() - 1 );  
...
//DS_FIX_FT_09_2007
  //uiBaseLayerId is no more discardable
    if(uiBaseLayerId != MSYS_UINT_MAX)                 //被当作参考层的discardable_flag参数为false
    {
      getLayerParameters(uiBaseLayerId).setNonDiscardable();
    }
//~DS_FIX_FT_09_2007

    if( uiBaseLayerId != MSYS_UINT_MAX )          //不能被丢弃
    {
	  ...
	  //把基本层的resize信息传递到当前层的ref resize信息中 
      ...
      if( rcCurrRP.m_iRefLayerFrmWidth != rcCurrRP.m_iRefLayerWidthInSamples )  //需要补偿等操作
      {
        Int iShift  = 1;
...
ErrVal EncoderCodingParameter::xReadLayerFromFile ( std::string&            rcFilename,
                                                    h264::LayerParameters&  rcLayer )
{//从每个dependency层读取配置文件参数       rcLayer 参数指的是当前的dependency layer
  std::string acTags[4];
...
// JVT-Q065 EIDR}
  ...
  for( UInt uiSMIdx = 0; uiSMIdx < 8; uiSMIdx++ )           //8个scalinglist
  {
    if( rcLayer.m_acScalMatFiles[uiSMIdx].empty() )   //每个dependency layer都有自己的8个scalinglist-------这里没有chroma_idc = 3？
    {
  ...
  //S051{
  if(cEncSIPFilename.length())  // SIP strategy 
  {
    rcLayer.setEncSIP(true);
    rcLayer.setInSIPFileName( (char*) cEncSIPFilename.c_str());    //记录帧间层预测不使用的POC
  }
  //S051}
  ...
  RNOK( xReadSliceGroupCfg( rcLayer)); //Slice group configuration file  读取slice group的配置文件  此处没用处
  //--ICU/ETRI FMO Implementation : FMO stuff end

  // ROI Config ICU/ETRI
  RNOK( xReadROICfg( rcLayer));        //暂时只知道ROI跟slice group有关  没有用到   相关的有m_uiROIID/m_uiSGID/m_uiSLID
