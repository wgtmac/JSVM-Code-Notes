...
protected:
  EncoderIoParameter            m_cEncoderIoParameter;                           //输入输出参数（输入视频文件  输出编码文件）
  EncoderCodingParameter*       m_pcEncoderCodingParameter;         //编码器参数
  h264::CreaterH264AVCEncoder*  m_pcH264AVCEncoder;                    //用来创建编码器参数
  WriteBitstreamToFile*         m_pcWriteBitstreamToFile;                        //写比特流文件
  WriteYuvToFile*               m_apcWriteYuv           [MAX_LAYERS];         //写重建文件
  ReadYuvFile*                  m_apcReadYuv            [MAX_LAYERS];          // 读视频文件
  ...
  UInt                          m_aauiCropping          [MAX_LAYERS][4];    //left right top bottom
