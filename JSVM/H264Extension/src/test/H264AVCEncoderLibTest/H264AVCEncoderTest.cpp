                                 ...
                                 Char** argv )
{
  ...
  RNOK( EncoderCodingParameter::create( m_pcEncoderCodingParameter ) );   //给编码参数成员m_pcEncoderCodingParameter动态分配空间
  if( Err::m_nOK != m_pcEncoderCodingParameter->init( argc, argv, m_cEncoderIoParameter.cBitstreamFilename ) )
  {																												// 上面的参数为传入参数，为了得到比特流文件名
  ...
  //    写重建文件：      WriteYuvToFile   m_apcWriteYuv[uiLayer] 
  //    读原始视频文件：ReadYuvFile       m_apcReadYuv [uiLayer]
  ...
  for( UInt uiLayer = 0; uiLayer < uiNumberOfLayers; uiLayer++ )
  {
    ...
    RNOKS( m_apcWriteYuv[uiLayer]->init( rcLayer.getOutputFilename() ) );    //获得输出重建文件的路径
    ...
    RNOKS( m_apcReadYuv[uiLayer]->init( rcLayer.getInputFilename        (),      //获得输入原始视频文件的路径
  ...
  if( m_pcEncoderCodingParameter->getAVCmode() )     //采用AVC模式，即普通264模式
  {
    RNOKS( WriteBitstreamToFile::create   ( m_pcWriteBitstreamToFile ) );
    RNOKS( m_pcWriteBitstreamToFile->init ( m_cEncoderIoParameter.cBitstreamFilename ) );    //比特流文件
  }
  else
  {
    m_cWriteToBitFileTempName                 = m_cEncoderIoParameter.cBitstreamFilename + ".temp";
    m_cWriteToBitFileName                     = m_cEncoderIoParameter.cBitstreamFilename;    //真正的比特流文件
    m_cEncoderIoParameter.cBitstreamFilename  = m_cWriteToBitFileTempName;               //用temp文件临时代替一下
    RNOKS( WriteBitstreamToFile::create   ( m_pcWriteBitstreamToFile ) );
    RNOKS( m_pcWriteBitstreamToFile->init ( m_cEncoderIoParameter.cBitstreamFilename ) ); 
  }

  //===== create encoder instance =====
  RNOK( h264::CreaterH264AVCEncoder::create( m_pcH264AVCEncoder ) );     //最重要的成员  编码器核心
  ...
  m_cBinDataStartCode.reset ();           //BinData   ---->  MemCont< UChar >类型
...
H264AVCEncoderTest::xGetNewPicBuffer ( PicBuffer*&  rpcPicBuffer,
                                       ...
                                       UInt         uiSize )
{
  if( m_acUnusedPicBufferList[uiLayer].empty() )           //未使用的PicBuffer的List已经使用完了  开辟一个新的
  {
  ...
  {
    rpcPicBuffer = m_acUnusedPicBufferList[uiLayer].popFront();            //从未使用的PicBuffer的List中取出一个
  }
...
{
  UInt                    uiWrittenBytes          = 0;                  //用来计算总共写的bytes
  ...
  PicBuffer*              apcOriginalPicBuffer    [MAX_LAYERS];             //原始图像
  PicBuffer*              apcReconstructPicBuffer [MAX_LAYERS];         //重建图像
  PicBufferList           acPicBufferOutputList   [MAX_LAYERS];           //MyList< PicBuffer* >    输出图像？
  PicBufferList           acPicBufferUnusedList   [MAX_LAYERS];
  ExtBinDataAccessorList  cOutExtBinDataAccessorList;                   //MyList< ExtBinDataAccessor* >    = MyList< MemAccessor<UChar>* >
  Bool                    bMoreSets;


  //===== initialization =====
  RNOK( m_pcH264AVCEncoder->init( m_pcEncoderCodingParameter ) );    //核心编码器的初始化

  // JVT-W043 {        据说只能对Base Layer进行RC   而对增强层不管
  ...
  if ( bRateControlEnable )
  {
    ...
    Int   iFullGOPs    = uiMaxFrame / iGOPSize;              //GOP组数量
  ...
  for( bMoreSets = true; bMoreSets;  )
  {
    ...
    cBinData.setMemAccessor( cExtBinDataAccessor );            //现在cBinData和cExtBinDataAccessor具有相同的指针内容UChar*

    h264::SequenceParameterSet* pcAVCSPS = NULL;              //SPS出现
    RNOK( m_pcH264AVCEncoder->writeParameterSets( &cExtBinDataAccessor, pcAVCSPS, bMoreSets ) );
		...
		{
      RNOK( m_pcWriteBitstreamToFile->writePacket       ( &m_cBinDataStartCode ) );     //先写starcode  0x00000001  区分NALUNIT
      RNOK( m_pcWriteBitstreamToFile->writePacket       ( &cExtBinDataAccessor ) );      //再写实际NAL内容
    ...
    // JVT-V068 {
/* luodan */
    if (pcAVCSPS)          //JVT-V068 {     HRD时   这里存了SPS的信息
    {
      cBinData.set( aucParameterSetBuffer, 1000 );
...
/* luodan */
  ...
  //===== determine parameters for required frame buffers =====   计算每层的图像高宽
  UInt  uiAllocMbX = 0;
  UInt  uiAllocMbY = 0;
  for( uiLayer = 0; uiLayer < uiNumLayers; uiLayer++ )
  {
    ...
    auiPicSize    [uiLayer]         = ((uiAllocMbX<<4)+2*YUV_X_MARGIN)*((uiAllocMbY<<4)+2*YUV_Y_MARGIN)*3/2;          //包括luma和chroma
    m_auiLumOffset[uiLayer]         = ((uiAllocMbX<<4)+2*YUV_X_MARGIN)* YUV_Y_MARGIN   + YUV_X_MARGIN;              //真正luma数据开始的位置
    m_auiCbOffset [uiLayer]         = ((uiAllocMbX<<3)+  YUV_X_MARGIN)* YUV_Y_MARGIN/2 + YUV_X_MARGIN/2 + uiSize;     //真正cb数据开始的位置
    m_auiCrOffset [uiLayer]         = ((uiAllocMbX<<3)+  YUV_X_MARGIN)* YUV_Y_MARGIN/2 + YUV_X_MARGIN/2 + 5*uiSize/4; //真正cr数据开始的位置
    m_auiStride   [uiLayer]         =  (uiAllocMbX<<4)+ 2*YUV_X_MARGIN;      //luma宽度
  }

  //===== loop over frames =====
  for( uiFrame = 0; uiFrame < uiMaxFrame; uiFrame++ )                   //对每一帧
  {
    //===== get picture buffers and read original pictures =====
    for( uiLayer = 0; uiLayer < uiNumLayers; uiLayer++ )                 //每一帧的每一层
    {
    ...
    //至此 读取了一帧在每一层的内容
    //===== call encoder =====
    RNOK( m_pcH264AVCEncoder->process( cOutExtBinDataAccessorList,                   // 这里还只是对每一个 Frame 进行 process  没有Layer的概念
