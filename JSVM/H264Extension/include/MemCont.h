...
public:
  ...
  public:
    MemContHelper& operator=( MemIf< T >& rcMemIf )            //用来复制内容到helper类中
    {
      rcMemIf.release( m_pcT, m_uiSize, m_pcOrigT, m_uiUsableSize );
      return *this;
    }
...
public:
  ...
  Void set( MemIf< T >& rcMemIf )
  {
    ROTV( this == &rcMemIf );
    if( m_pcOrigT ) { delete[] m_pcOrigT; }
    rcMemIf.release( m_pcT, m_uiSize, m_pcOrigT, m_uiUsableSize );  //释放参数内容
  }
  ...
  //把MemCont<T>类的成员copy到参数MemAccessor< T >& rcMemAccessor 中
