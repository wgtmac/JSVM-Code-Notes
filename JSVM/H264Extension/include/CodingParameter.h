...
//从这里可以看出  SNR若采用MGS，则每个SNR层都有一部分MV；若采用CGS，则只有第一层有MV
...
public:
  ...
  Double                    m_adQpModeDecision[MAX_DSTAGES];		// QP for TLs（ME MD）
  Double                    m_dQpModeDecisionLP;							// QP for KeyPicture（ME MD）
  ...
  //----- derived parameters -----
  UInt                      m_uiDecompositionStages;			   // 时间层层数
  UInt                      m_uiNotCodedStages    ;				  //时间层 不编码的层数（输入输出码率比）
  UInt                      m_uiTemporalResolution;					// 输入文件的采样比
  UInt                      m_uiFrameDelay;
...
// JVT-Q065 EIDR}
  ...
  UInt       m_uiMGSVect[16];             //指示每个SNR层的MV个数
  ...
  //S051}
//JVT-T054{
  UInt                      m_uiLayerCGSSNR;					// dependency_id
  UInt                      m_uiQualityLevelCGSSNR;			// quality_id
...
//JVT-T054}
  ...
  Double  m_adDeltaQPTLevel[MAX_TEMP_LEVELS];			// 每一个TL的ExplicitQPCascading

  UInt    m_uiIPCMRate;
  std::string m_acScalMatFiles      [8];								// ScalingList的文件
  UChar       m_aaucScalingMatrices [8][64];						// 8个ScalingList
  UInt  m_uiBiPred8x8Disable;
  UInt  m_uiMCBlks8x8Disable;
  UInt  m_uiBotFieldFirst;
  UInt  m_uiUseLongTerm;
  UInt  m_uiPicCodingType;					// 1时没有B帧
  UInt  m_uiNumDependentDId;			// D_id
  UInt  m_uiProfileIdc;
  UInt  m_uiLevelIdc;
  Bool  m_bIntraOnly;
...
protected:
  ...
  UInt                      m_uiGOPSize;
  UInt                      m_uiDecompositionStages;				// GOP的stage，就是时间层
...
// JVT-U116,W062 LMI
  ...
  //JVT-U106 Behaviour at slice boundaries{
  UInt                      m_uiCIUFlag;					//  ConstrainedIntraUp          slice边界影响IntraBase的上采样和去块滤波
  //JVT-U106 Behaviour at slice boundaries}
