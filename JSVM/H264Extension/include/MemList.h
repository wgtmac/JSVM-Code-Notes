    ...
    if( (0 != rcMemList.m_uiSize) && (0 != rcMemList.m_cMemAccessorList.size()) )
    {
      ...
      for( pcPair = rcMemList.m_cMemAccessorList.begin(); pcPair != rcMemList.m_cMemAccessorList.end(); pcPair++  )
      {
        if( (NULL != pcPair->data() ) && (0 != pcPair->size()) )
        {
          ...
          m_cMemAccessorList.push_back( MemAccessor< T >( pcT, pcPair->size(), pcT, pcPair->size() ) );  //与MemAccessList<T>有区别
        }
        else
        {
...
public:
  ...
  Void set( T* pcT, UInt uiSize, T* pcOrigT=NULL, UInt uiUsableSize=0 )
  {
    ...
    for( pcPair = m_cMemAccessorList.begin(); pcPair != m_cMemAccessorList.end(); pcPair++  )
    {
      ...
      {
        m_cMemAccessorList.erase( pcPair );             //erase掉的目的是防止下面的deleteData()操作delete掉了这个元素
        break;
      }
    }
    deleteData();
  ...
  Void release( T*& rpcT, UInt& ruiSize )        //把list中所有T*内容连接起来赋给rpcT，并清空
  {
    if( 0 == m_uiSize )
    {
      rpcT = NULL;
    ...
    MemAccessorListIterator pcPair;
    if( m_cMemAccessorList.size() > 1 )          //list多余一个元素
    {
      UInt uiPos = 0;
      rpcT = new T[ruiSize];
      ROTV( NULL == rpcT );
  ...
  Void release( T*& rpcT, UInt& ruiSize, T*& rpcOrigT, UInt& ruiUsableSize )
  {
    ...
    MemAccessorListIterator pcPair;
    if( m_cMemAccessorList.size() > 1 )              //？？？？？为什么多于一个元素的list 的rpcOrigT跟rpcT一样     而只有一个的却分开rpcT = pcPair->data() 和  rpcOrigT = pcPair->origData();
    {
      UInt uiPos = 0;
      rpcT = new T[ruiSize];
      ROTV( NULL == rpcT );
