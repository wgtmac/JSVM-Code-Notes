...
public:
  ...
  UInt          uiFrameIdCol;     //建队列时List1的最后一帧
  RefFrameList  acRefFrameListME[2];           //运动估计得到MV用的队列，可能为RC也可能为EL的队列
  RefFrameList  acRefFrameListMC[2];           //运动补偿得到residual用的队列，可能为RC也可能为EL的队列
  RefFrameList  acRefFrameListRC[2];            //Std的参考队列
};
