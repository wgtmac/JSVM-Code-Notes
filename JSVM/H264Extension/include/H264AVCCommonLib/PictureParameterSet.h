...
protected:
  ...
  UInt          m_uiPicParameterSetId;     //pic_parameter_set_rbsp
  UInt          m_uiSeqParameterSetId;    //pic_parameter_set_rbsp
  Bool          m_bEntropyCodingModeFlag;        //pic_parameter_set_rbsp
  Bool          m_bPicOrderPresentFlag;                 //pic_parameter_set_rbsp    --》  bottom_field_pic_order_in_frame_present_flag


  //--ICU/ETRI FMO Implementation : FMO stuff start
  UInt          m_uiNumSliceGroupsMinus1;           //pic_parameter_set_rbsp
  UInt          m_uiSliceGroupMapType;                  //pic_parameter_set_rbsp
  UInt          m_uiRunLengthMinus1[MAXNumSliceGroupsMinus1];          //pic_parameter_set_rbsp
  UInt          m_uiTopLeft[MAXNumSliceGroupsMinus1];           //pic_parameter_set_rbsp
  UInt          m_uiBottomRight[MAXNumSliceGroupsMinus1];  //pic_parameter_set_rbsp
  Bool          m_bSliceGroupChangeDirection_flag;       //pic_parameter_set_rbsp
  UInt          m_uiSliceGroupChangeRateMinus1;           //pic_parameter_set_rbsp
  UInt          m_uiNumSliceGroupMapUnitsMinus1;       //pic_parameter_set_rbsp
  UInt          m_uiSliceGroupChangeCycle;              //???????
  UInt          m_uiSliceGroupIdArraySize;     //pic_parameter_set_rbsp     --->    slice_group_id[ i ]的size
  UInt*         m_pauiSliceGroupId;                //pic_parameter_set_rbsp     --->    slice_group_id[ i ]的指针
  //--ICU/ETRI FMO Implementation : FMO stuff end

  // SVC没有SI和SP的概念  因此PPS里没有pic_init_qs_minus26

  UInt          m_auiNumRefIdxActive[2];           //pic_parameter_set_rbsp
  Bool          m_bWeightedPredFlag;                //pic_parameter_set_rbsp
  UInt          m_uiWeightedBiPredIdc;               //pic_parameter_set_rbsp
  UInt          m_uiPicInitQp;                                 //pic_parameter_set_rbsp   -->  pic_init_qp_minus26
  Int           m_iChromaQpIndexOffset;               //pic_parameter_set_rbsp
  Bool          m_bDeblockingFilterParametersPresentFlag;          //pic_parameter_set_rbsp
  Bool          m_bConstrainedIntraPredFlag;         //pic_parameter_set_rbsp
  Bool          m_bTransform8x8ModeFlag;             //pic_parameter_set_rbsp
  Bool          m_bPicScalingMatrixPresentFlag;      //pic_parameter_set_rbsp
  ScalingMatrix m_cPicScalingMatrix;                   //pic_parameter_set_rbsp
  Int           m_iSecondChromaQpIndexOffset;        //pic_parameter_set_rbsp
  Bool          m_bRedundantPicCntPresentFlag;  //JVT-Q054 Red. Picture u(1)           //pic_parameter_set_rbsp
	Bool          m_bRedundantKeyPicCntPresentFlag;  //JVT-W049                    SVC新增KeyPicture
  Bool          m_bEnableRedundantKeyPicCntPresentFlag;  //JVT-W049           SVC新增KeyPicture

  Bool          m_bReferencesSubsetSPS;              //是否采用 SPS的subset?
};
