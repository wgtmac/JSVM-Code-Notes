...
protected:
  ...
  public:
    ...
    B4x4Idx           m_cIdx;                    //该8x8块左上角的4x4块号
    PredWeight        m_acPW[2];         //加权预测的权 ?
    const PredWeight* m_apcPW[2]; //加权预测的权 ?
    YuvPicBuffer*     m_apcRefBuffer[2];    //用来参考的图像
    Mv3D              m_aacMv[2][6];      //   [LIST0/1] [4个  4x4 luma  + cb 4x4+ cr 4x4]
    Mv3D              m_aacMvd[2][6];  //    [LIST0/1] [4个  4x4 luma  + cb 4x4+ cr 4x4]      differential motion vector   
	Short             m_sChromaOffset[2];     //FIELD时 LUMA=>CHROMA的修正
  };
