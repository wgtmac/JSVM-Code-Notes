...
private:
  static const UInt m_auiBlk2Part[16];                // 16个 4x4块 属于哪个 8x8块
  SChar             m_ascRefIdx   [4];                        // 4个 8x8块 的ref_idx
  RefPicIdc         m_acRefPicIdc [4];
  UShort            m_usMotPredFlags;              // MotionPredFlag     question ??????
};
...
{   // MbMotionData : public MbMvData
  rcMv3D.set( m_acMv[ cIdx.b4x4() ], m_ascRefIdx[ m_auiBlk2Part[ cIdx.b4x4()] ] );     // 从对应的 A/B/C/D4x4块中读取出Mv和ref_idx
