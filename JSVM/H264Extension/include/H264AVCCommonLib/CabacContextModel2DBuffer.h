...
public:
  ...
  ErrVal initBuffer( Short* psCtxModel, Int iQp )
  {
    ...
    {
      m_pcCContextModel[n].init( psCtxModel + 2*n, iQp );     //对每个上下文进行初始化
    }
    return Err::m_nOK;
  }
	//JVT-X046 {
  ...
  //JVT-X046 }
protected:
  CabacContextModel*  m_pcCContextModel;     //一维数组 保存该句法元素所有上下文的概率状态
  const UInt          m_uiSizeX;                 // 该句法元素上下文个数（每种块）
  const UInt          m_uiSizeY;                 // 该句法元素块类型
};
