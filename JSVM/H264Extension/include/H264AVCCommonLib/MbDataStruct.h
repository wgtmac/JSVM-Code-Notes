...
public:
  ...
  {
    if( bReset )             // uiPos对应4x4块号
    {
      m_uiBCBP &= ~(1 << uiPos);      //先清零
    }
    m_uiBCBP   |= (uiBit << uiPos);    //再写入
  }
...
protected:
  ...
  UInt      m_uiMbAddr;  
  UInt      m_uiMapUnit;
  SliceType m_eSliceType;                           //Slice Type
  Bool      m_bBLSkipFlag;
  MbMode    m_eMbMode;
  UInt      m_uiMbCbp;                               // cbp，  6位
  UInt      m_uiBCBP;                                   //整个宏块的 coded_block_flag  共16位
  UShort    m_usFwdBwd;                                // 指示每个8x8块 前向、后向预测的情况
  UChar     m_ucChromaPredMode;                     //Chroma Intra 预测模式
  BlkMode   m_aBlkMode[4];                                // 每个8x8块 划分的类型
  SChar     m_ascIPredMode[16];                          //Luma Intra 预测模式
  UChar     m_ucQp;
  UChar     m_ucQp4LF;
