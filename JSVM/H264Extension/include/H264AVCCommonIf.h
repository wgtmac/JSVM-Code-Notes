...
private:
  Int   m_iCoeffValue; // more than 16 bit required for chroma DC       量化后的系数
  Int   m_iLevelValue; // more than 16 bit required for chroma DC       反量化后的系数
  XPel  m_sPred;         //                                                                               重建值
};

template< class T >
class MyList : public std::list< T >      //注意：此处T类型必须为指针类型才行
{
public:
  typedef typename std::list<T>::iterator MyIterator;

  //插入MyList类型的list到末尾
  MyList& operator += ( const MyList& rcMyList) { if( ! rcMyList.empty() ) { insert( this->end(), rcMyList.begin(), rcMyList.end());} return *this; }
  //弹出并得到尾端元素
  T popBack()                           { T cT = this->back(); this->pop_back(); return cT;  }
  //弹出并得到首端元素
  T popFront()                          { T cT = this->front(); this->pop_front(); return cT; }
  Void pushBack( const T& rcT )         { if( sizeof(T) == sizeof(void*) ) { if( rcT != NULL ){ push_back( rcT);} } }         //T必须是一个指针类型
...
class ExtBinDataAccessor : public BinDataAccessor            //就是 MemAccessor<UChar> 多了个m_pcMediaPacket
{
public:
...
public:
  ...
  operator Pel*()  { return m_pcBuffer; }               //类型转换   将该类做  (Pel*)的强制转换可以得到m_pcBuffer的值
  Void* getMediaPacket() { return m_pcMediaPacket; }
...
private:
  Void*          m_pcMediaPacket;
  Pel*            m_pcBuffer;                 //缓存  /  缓冲区
  Int              m_iInUseCout;             //使用计数器
  Int64			 m_i64Cts;
  PicStruct   m_ePicStruct;               //图像结构----PS_FRAME
  Int              m_iTopPoc;                   //顶场POC
  Int              m_iBotPoc;                    //底场POC
  Int             m_iFramePoc;                //帧POC
  UInt           m_uiIdrPicId;                 //IDR帧的ID
  Bool          m_bFieldCoded;
};
